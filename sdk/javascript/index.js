/**
 * @rustyhand/sdk — Official JavaScript client for the RustyHand Agent OS REST API.
 *
 * Usage:
 *   const { RustyHand } = require("@rustyhand/sdk");
 *   const client = new RustyHand("http://localhost:3000");
 *
...
class RustyHandError extends Error {
  constructor(message, status, body) {
    super(message);
    this.name = "RustyHandError";
    this.status = status;
    this.body = body;
  }
}
...
class RustyHand {
  /**
   * @param {string} baseUrl - RustyHand server URL (e.g. "http://localhost:3000")
   * @param {object} [opts]
  ...
  async _request(method, path, body) {
    ...
    if (!res.ok) {
      ...
      throw new RustyHandError("HTTP " + res.status + ": " + text, res.status, text);
    }
...
class AgentResource {
  ...
  async upload(id, file, filename) {
    ...
    if (!res.ok) throw new RustyHandError("Upload failed: " + res.status, res.status);
    return res.json();
  }
...
class CronResource {
  ...
  async list() {
    var data = await this._c._request("GET", "/api/cron/jobs");
    return data.jobs || [];
  }

  async create(schedule) {
    var data = await this._c._request("POST", "/api/cron/jobs", schedule);
  ...
  async update(id, schedule) {
    var data = await this._c._request("PUT", "/api/cron/jobs/" + id, schedule);
    if (data && typeof data.result === "string") {
      try { return JSON.parse(data.result); } catch (_) {}
    }
    return data;
  ...
  async run(id) {
    return this._c._request("POST", "/api/cron/jobs/" + id + "/run");
  }
}

// ── Exports ─────────────────────────────────────────────────────

module.exports = { RustyHand: RustyHand, RustyHandError: RustyHandError };
