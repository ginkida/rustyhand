"""
RustyHand Python Client — REST API client for controlling RustyHand remotely.

Usage:

    from rustyhand_client import RustyHand

    client = RustyHand("http://localhost:3000")
...
Note: This is the REST API *client* library.
      For writing Python agents that run inside RustyHand, see rustyhand_sdk.py instead.
"""
...
class RustyHandError(Exception):
...
class _Resource:
    def __init__(self, client: "RustyHand"):
        self._c = client


class RustyHand:
    """RustyHand REST API client. Zero dependencies — uses only stdlib urllib."""
    ...
    def _request(self, method: str, path: str, body: Any = None) -> Any:
        ...
        try:
            ...
            raise RustyHandError(f"HTTP {e.code}: {body_text}", e.code, body_text) from e
...
class _CronResource(_Resource):

    def list(self):
        data = self._c._request("GET", "/api/cron/jobs")
        return data.get("jobs", []) if isinstance(data, dict) else data

    def create(self, **schedule):
        data = self._c._request("POST", "/api/cron/jobs", schedule)
    ...
    def update(self, schedule_id: str, **schedule):
        data = self._c._request("PUT", f"/api/cron/jobs/{schedule_id}", schedule)
        if isinstance(data, dict) and isinstance(data.get("result"), str):
            try:
                return json.loads(data["result"])
            except json.JSONDecodeError:
    ...
    def delete(self, schedule_id: str):
        return self._c._request("DELETE", f"/api/cron/jobs/{schedule_id}")

    def run(self, schedule_id: str):
        return self._c._request("POST", f"/api/cron/jobs/{schedule_id}/run")
