...
/// Result of converting a SKILL.md into RustyHand format.
#[derive(Debug, Clone)]
pub struct ConvertedSkillMd {
    ...
    /// Tool name translations applied (openclaw_name → rusty_hand_name).
...
/// Full conversion of a SKILL.md directory to RustyHand format.
///
...
pub fn convert_skillmd(dir: &Path) -> Result<ConvertedSkillMd, SkillError> {
    ...
    // Rewrite OpenClaw/Codex paths to RustyHand equivalents
    let body = body
        .replace("~/.codex/skills/", "~/.rustyhand/skills/")
        .replace("~/.codex/", "~/.rustyhand/");
    ...
    if let Some(ref meta) = frontmatter.metadata.openclaw {
        ...
        // Convert commands to RustyHand tool definitions
        for cmd in &meta.commands {
            ...
            let rusty_hand_name = if let Some(mapped) = tool_compat::map_tool_name(&cmd.name) {
                tool_translations.push((cmd.name.clone(), mapped.to_string()));
                mapped.to_string()
            } else if tool_compat::is_known_rusty_hand_tool(&cmd.name) {
                cmd.name.clone()
            } else {
...
/// Convert an in-memory SKILL.md string into RustyHand format.
///
...
/// Convert an OpenClaw Node.js skill directory into an RustyHand SkillManifest.
///
...
/// Write an RustyHand skill.toml manifest for an OpenClaw skill.
pub fn write_rusty_hand_manifest(dir: &Path, manifest: &SkillManifest) -> Result<(), SkillError> {
...
# Git Helper instructions"#;
    ...
    #[test]
    fn test_convert_skillmd_rewrites_codex_paths() {
        let dir = TempDir::new().unwrap();
        let content = r#"---
name: Image Gen
...
Scripts are located at ~/.codex/skills/image-gen/scripts/generate_image.py
...
"#;
        ...
        assert!(
            !converted.prompt_context.contains("~/.codex/"),
            "codex paths should be rewritten"
        );
        assert!(
            converted
                .prompt_context
                .contains("~/.rustyhand/skills/image-gen/scripts/generate_image.py"),
