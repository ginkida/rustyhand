//! RustyHand daemon server — boots the kernel and serves the HTTP API.
...
/// Daemon info written to `~/.rustyhand/daemon.json` so the CLI can find us.
...
///
/// This is extracted from `run_daemon()` so that embedders (e.g. rusty-hand-desktop)
...
pub async fn build_router(
    ...
    let app = Router::new()
        ...
        .route(
            "/api/memory/export",
            axum::routing::get(routes::export_memory_backup),
        )
        .route(
            "/api/memory/import",
            axum::routing::post(routes::import_memory_backup),
        )
        .route("/api/auth/me", axum::routing::get(routes::auth_me))
        .route(
            "/api/auth/users",
            axum::routing::get(routes::list_auth_users),
        )
        // Trigger endpoints
        .route(
            "/api/triggers",
...
/// Start the RustyHand daemon: boot kernel + HTTP API server.
///
...
pub async fn run_daemon(
    ...
    // SECURITY: warn if binding to a non-loopback address
    if !addr.ip().is_loopback() {
        let api_key_set = !kernel.config.api_key.trim().is_empty() || kernel.auth.has_api_keys();
        if api_key_set {
            warn!(
                "API server binding to {addr} (non-loopback). \
                 Authentication is enabled — remote requests require a valid API key."
            );
        } else {
            warn!(
                "API server binding to {addr} (non-loopback) with NO API key configured. \
                 All remote requests will be rejected (403). \
                 Set api_key in config.toml to allow remote access, \
                 or use 127.0.0.1 to restrict to localhost."
            );
        }
    }

    info!("RustyHand API server listening on http://{addr}");
