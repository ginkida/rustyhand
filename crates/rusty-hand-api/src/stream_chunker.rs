...
impl StreamChunker {
    ...
    pub fn try_flush(&mut self) -> Option<String> {
        let buffered_chars = char_count(&self.buffer);
        if buffered_chars < self.min_chunk_chars {
            return None;
        }

        // If inside a code fence and under max, wait for fence to close
        if self.in_code_fence && buffered_chars < self.max_chunk_chars {
            return None;
        }
        ...
        let search_range = byte_index_at_char(&self.buffer, self.min_chunk_chars)
            ..byte_index_at_char(&self.buffer, self.max_chunk_chars.min(buffered_chars));
        ...
        if buffered_chars >= self.max_chunk_chars {
            let break_at = byte_index_at_char(&self.buffer, self.max_chunk_chars);
            let chunk = self.buffer[..break_at].to_string();
            self.buffer = self.buffer[break_at..].to_string();
            return Some(chunk);
        }
...
/// Convert a character offset to a byte index without splitting UTF-8.
...
/// Find the last occurrence of a pattern within a byte range on char boundaries.
...
mod tests {
    ...
    #[test]
    fn test_unicode_forced_break_respects_char_boundaries() {
        let mut chunker = StreamChunker::new(1, 5);
        let text = "🙂🙂🙂🙂🙂🙂";
        chunker.push(text);

        let first = chunker.try_flush().unwrap();
        let rest = chunker.flush_remaining().unwrap();
