...
use rusty_hand_runtime::llm_errors;
...
pub async fn agent_ws(
    ...
    uri: axum::http::Uri,
    auth_user: Option<Extension<crate::middleware::AuthenticatedUser>>,
) -> impl IntoResponse {
    // WebSocket upgrades should already pass through HTTP auth middleware, but
    // keep a local fallback for routers that mount this handler directly.
    let auth_enabled =
        !state.kernel.config.api_key.trim().is_empty() || state.kernel.auth.has_api_keys();
    if auth_user.is_none() && auth_enabled {
        let header_auth = crate::middleware::bearer_token(&headers)
            .and_then(|token| crate::middleware::authenticate_token(&state, token))
            .is_some();
        let query_auth = crate::middleware::query_token(&uri)
            .and_then(|token| crate::middleware::authenticate_token(&state, token))
            .is_some();
...
async fn handle_agent_ws(
    ...
    // Spawn background task: keepalive pings + periodic agent list updates
    ...
    let update_handle = tokio::spawn(async move {
        ...
        loop {
            ...
            // Always send a WS-level ping to keep NATs/proxies alive
            {
...
async fn handle_text_message(
    ...
    match msg_type {
                        ...
                        &serde_json::json!({
                            ...
                            "message": format!(
                                "**Vision not supported** — the current model `{}` cannot analyze images. \
                                 Switch to a vision-capable model (e.g. `gemini-2.5-flash`, `claude-sonnet-4-6`, `gpt-4o`) \
        ...
        "command" => {
            ...
            send_json_lossy(sender, &response).await;
        }
        "ping" => {
            send_json_lossy(sender, &serde_json::json!({"type": "pong"})).await;
        }
        other => {
...
async fn handle_command(
    ...
    match cmd {
        ...
        "context" => match state.kernel.context_report(agent_id) {
            Ok(report) => {
                let formatted = rusty_hand_runtime::compactor::format_context_report(&report);
                serde_json::json!({
                    "type": "command_result",
                    "command": cmd,
                    "message": formatted,
        ...
        "peers" => {
            let msg = if !state.kernel.config.network_enabled {
                "RHP network disabled.".to_string()
            } else {
...
fn map_stream_event(event: &StreamEvent, verbose: VerboseLevel) -> Option<serde_json::Value> {
    match event {
        ...
        StreamEvent::ToolExecutionProgress { name, elapsed_secs } => Some(serde_json::json!({
            "type": "tool_progress",
            "name": name,
...
/// Fire-and-forget variant of [`send_json`] that logs delivery failures
/// instead of silently dropping them. Use this for non-critical messages
/// where the caller doesn't need to react to send errors (e.g., typing
/// indicators, streaming deltas).
async fn send_json_lossy(
    sender: &Arc<Mutex<SplitSink<WebSocket, Message>>>,
    value: &serde_json::Value,
) {
    if let Err(e) = send_json(sender, value).await {
        debug!("WebSocket send failed (client likely disconnected): {e}");
    }
}
...
///
/// Uses the proper LLM error classifier from `rusty_hand_runtime::llm_errors`
/// for comprehensive 20-provider coverage with actionable advice.
fn classify_streaming_error(err: &rusty_hand_kernel::error::KernelError) -> String {
