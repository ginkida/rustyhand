...
//!
//! Allows any OpenAI-compatible client library to talk to RustyHand agents.
//! The `model` field resolves to an agent (by name, UUID, or `rustyhand:<name>`),
...
fn resolve_agent(state: &AppState, model: &str) -> Option<(AgentId, String)> {
    // 1. "rustyhand:<name>" → find agent by name
    if let Some(name) = model.strip_prefix("rustyhand:") {
...
pub async fn list_models(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let models: Vec<ModelObject> = agents
        .iter()
        .map(|e| ModelObject {
            id: format!("rustyhand:{}", e.name),
            object: "model",
            created,
            owned_by: "rustyhand".to_string(),
        })
        .collect();
