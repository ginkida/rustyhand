...
//!
//! RustyHand uses capability-based security: an agent can only perform actions
...
impl CapabilityCheck {
    ...
    pub fn require(&self) -> Result<(), crate::error::RustyHandError> {
        match self {
            Self::Granted => Ok(()),
            Self::Denied(reason) => Err(crate::error::RustyHandError::CapabilityDenied(
                reason.clone(),
            )),
        }
    }
