//! Shared tool name mappings between OpenClaw and RustyHand.
//!
//! These mappings are used by both the migration engine and the skill system
//! to normalize OpenClaw tool names into RustyHand equivalents.

/// Map an OpenClaw tool name to its RustyHand equivalent.
///
/// Returns `None` if the name has no known mapping (may already be
/// an RustyHand tool name — check with [`is_known_rusty_hand_tool`]).
...
/// Check if a tool name is a known RustyHand built-in tool.
pub fn is_known_rusty_hand_tool(name: &str) -> bool {
    matches!(
        name,
        "file_read"
            | "file_write"
...
mod tests {
    ...
    #[test]
    fn test_is_known_rusty_hand_tool() {
        ...
        for tool in &known {
            assert!(
                is_known_rusty_hand_tool(tool),
                "Expected {tool} to be known"
            );
        }

        // Unknown
        assert!(!is_known_rusty_hand_tool("unknown"));
        assert!(!is_known_rusty_hand_tool("Read"));
        assert!(!is_known_rusty_hand_tool("Bash"));
    }
}
