...
use dashmap::DashMap;
use rusty_hand_types::agent::{AgentEntry, AgentId, AgentMode, AgentState};
...
impl AgentRegistry {
    ...
    pub fn register(&self, entry: AgentEntry) -> RustyHandResult<()> {
        if self.name_index.contains_key(&entry.name) {
            return Err(RustyHandError::AgentAlreadyExists(entry.name.clone()));
        }
        let id = entry.id;
    ...
    /// Update agent state.
    pub fn set_state(&self, id: AgentId, state: AgentState) -> RustyHandResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn set_mode(&self, id: AgentId, mode: AgentMode) -> RustyHandResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| RustyHandError::AgentNotFound(id.to_string()))?;
    ...
    pub fn remove(&self, id: AgentId) -> RustyHandResult<AgentEntry> {
        let (_, entry) = self
            .agents
            .remove(&id)
            .ok_or_else(|| RustyHandError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_session_id(
        &self,
        id: AgentId,
        new_session_id: rusty_hand_types::agent::SessionId,
    ) -> RustyHandResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_identity(
        &self,
        id: AgentId,
        identity: rusty_hand_types::agent::AgentIdentity,
    ) -> RustyHandResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_model(&self, id: AgentId, new_model: String) -> RustyHandResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| RustyHandError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_system_prompt(&self, id: AgentId, new_prompt: String) -> RustyHandResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| RustyHandError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_description(&self, id: AgentId, new_desc: String) -> RustyHandResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| RustyHandError::AgentNotFound(id.to_string()))?;
    ...
    /// Update an agent's dashboard grouping label.
    pub fn update_group(&self, id: AgentId, group: Option<String>) -> RustyHandResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| RustyHandError::AgentNotFound(id.to_string()))?;
        entry.manifest.group = group;
        entry.last_active = chrono::Utc::now();
        Ok(())
    }

    /// Mark an agent's onboarding as complete.
    pub fn mark_onboarding_complete(&self, id: AgentId) -> RustyHandResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| RustyHandError::AgentNotFound(id.to_string()))?;
