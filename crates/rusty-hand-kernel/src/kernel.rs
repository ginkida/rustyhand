//! RustyHandKernel — assembles all subsystems and provides the main API.
...
use rusty_hand_runtime::agent_loop::{run_agent_loop, run_agent_loop_streaming, AgentLoopResult};
use rusty_hand_runtime::audit::AuditLog;
use rusty_hand_runtime::drivers;
use rusty_hand_runtime::kernel_handle::{self, KernelHandle};
use rusty_hand_runtime::llm_driver::{CompletionRequest, DriverConfig, LlmDriver, StreamEvent};
use rusty_hand_runtime::python_runtime::{self, PythonConfig};
use rusty_hand_runtime::routing::ModelRouter;
use rusty_hand_runtime::sandbox::{SandboxConfig, WasmSandbox};
use rusty_hand_runtime::tool_runner::builtin_tool_definitions;
use rusty_hand_types::agent::*;
...
use rusty_hand_types::memory::Memory;
...
pub struct RustyHandKernel {
    ...
    /// Runtime budget configuration (mutable via API, independent of on-disk config).
    runtime_budget: std::sync::RwLock<rusty_hand_types::config::BudgetConfig>,
    ...
    pub model_catalog: std::sync::RwLock<rusty_hand_runtime::model_catalog::ModelCatalog>,
    /// Skill registry for plugin skills (RwLock for hot-reload on install/uninstall).
    pub skill_registry: std::sync::RwLock<rusty_hand_skills::registry::SkillRegistry>,
    ...
    pub mcp_connections: tokio::sync::Mutex<Vec<rusty_hand_runtime::mcp::McpConnection>>,
    ...
    pub a2a_task_store: rusty_hand_runtime::a2a::A2aTaskStore,
    /// Discovered external A2A agent cards.
    pub a2a_external_agents: std::sync::Mutex<Vec<(String, rusty_hand_runtime::a2a::AgentCard)>>,
    /// Web tools context (multi-provider search + SSRF-protected fetch + caching).
    pub web_ctx: rusty_hand_runtime::web_search::WebToolsContext,
    /// Browser automation manager (agent-browser CLI sessions).
    pub browser_ctx: rusty_hand_runtime::browser::BrowserManager,
    /// Media understanding engine (image description, audio transcription).
    pub media_engine: rusty_hand_runtime::media_understanding::MediaEngine,
    /// Text-to-speech engine.
    pub tts_engine: rusty_hand_runtime::tts::TtsEngine,
    ...
    pub embedding_driver:
        Option<Arc<dyn rusty_hand_runtime::embedding::EmbeddingDriver + Send + Sync>>,
    /// Extension/integration registry (bundled MCP templates + install state).
    pub extension_registry: std::sync::RwLock<rusty_hand_extensions::registry::IntegrationRegistry>,
    /// Integration health monitor.
    pub extension_health: rusty_hand_extensions::health::HealthMonitor,
    /// Effective MCP server list (manual config + extension-installed, merged at boot).
    pub effective_mcp_servers:
        std::sync::RwLock<Vec<rusty_hand_types::config::McpServerConfigEntry>>,
    ...
    pub bindings: std::sync::Mutex<Vec<rusty_hand_types::config::AgentBinding>>,
    /// Broadcast configuration.
    pub broadcast: rusty_hand_types::config::BroadcastConfig,
    /// Auto-reply engine.
    pub auto_reply_engine: crate::auto_reply::AutoReplyEngine,
    /// Plugin lifecycle hook registry.
    pub hooks: rusty_hand_runtime::hooks::HookRegistry,
    /// Persistent process manager for interactive sessions (REPLs, servers).
    pub process_manager: Arc<rusty_hand_runtime::process_manager::ProcessManager>,
    /// RHP peer registry — tracks connected peers.
    peer_registry: std::sync::RwLock<Option<rusty_hand_wire::PeerRegistry>>,
    /// RHP peer node — the local networking node.
    peer_node: std::sync::RwLock<Option<Arc<rusty_hand_wire::PeerNode>>>,
...
pub struct DeliveryTracker {
    receipts: dashmap::DashMap<AgentId, Vec<rusty_hand_channels::types::DeliveryReceipt>>,
}
...
impl DeliveryTracker {
    ...
    pub fn record(&self, agent_id: AgentId, receipt: rusty_hand_channels::types::DeliveryReceipt) {
        ...
        drop(entry);
        let mut total: usize = self.receipts.iter().map(|e| e.value().len()).sum();
        while total > Self::MAX_RECEIPTS {
            // Evict from the agent with the oldest receipt timestamp.
            let oldest_agent = self
                .receipts
                .iter()
                .filter_map(|e| e.value().first().map(|r| (*e.key(), r.timestamp)))
                .min_by_key(|(_, ts)| *ts)
                .map(|(agent_id, _)| agent_id);

            let Some(oldest_agent) = oldest_agent else {
                break;
            };

            if let Some(mut oldest) = self.receipts.get_mut(&oldest_agent) {
                let to_remove = total - Self::MAX_RECEIPTS;
                let drain = to_remove.min(oldest.len());
                oldest.drain(..drain);
                total -= drain;
                if oldest.is_empty() {
                    drop(oldest);
                    self.receipts.remove(&oldest_agent);
                }
            } else {
                break;
            }
    ...
    pub fn get_receipts(
        ...
        agent_id: AgentId,
        limit: usize,
    ) -> Vec<rusty_hand_channels::types::DeliveryReceipt> {
        self.receipts
            .get(&agent_id)
    ...
    pub fn sent_receipt(
        channel: &str,
        recipient: &str,
    ) -> rusty_hand_channels::types::DeliveryReceipt {
        rusty_hand_channels::types::DeliveryReceipt {
            message_id: uuid::Uuid::new_v4().to_string(),
            channel: channel.to_string(),
            recipient: Self::sanitize_recipient(recipient),
            status: rusty_hand_channels::types::DeliveryStatus::Sent,
    ...
    pub fn failed_receipt(
        ...
        rusty_hand_channels::types::DeliveryReceipt {
            ...
            channel: channel.to_string(),
            recipient: Self::sanitize_recipient(recipient),
            status: rusty_hand_channels::types::DeliveryStatus::Failed,
            timestamp: chrono::Utc::now(),
...
fn read_identity_file(workspace: &Path, filename: &str) -> Option<String> {
    ...
    if content.len() > MAX_IDENTITY_FILE_BYTES {
        Some(truncate_utf8_bytes(&content, MAX_IDENTITY_FILE_BYTES))
    } else {
        Some(content)
    }
}
...
impl RustyHandKernel {
    ...
    pub fn boot_with_config(mut config: KernelConfig) -> KernelResult<Self> {
        ...
        match config.mode {
            KernelMode::Stable => {
                info!("Booting RustyHand kernel in STABLE mode — conservative defaults enforced");
            }
            KernelMode::Dev => {
                warn!("Booting RustyHand kernel in DEV mode — experimental features enabled");
            }
            KernelMode::Default => {
                info!("Booting RustyHand kernel...");
            }
        }
        ...
        let db_path = config
            ...
            .sqlite_path
            .clone()
            .unwrap_or_else(|| config.data_dir.join("rustyhand.db"));
        let memory = Arc::new(
        ...
        let driver: Arc<dyn LlmDriver> = if !config.fallback_providers.is_empty() {
            ...
            if chain.len() > 1 {
                Arc::new(rusty_hand_runtime::drivers::fallback::FallbackDriver::new(
                    chain,
                ))
            } else {
                primary_driver
        ...
        let metering = Arc::new(MeteringEngine::new(Arc::new(
            rusty_hand_memory::usage::UsageStore::new(memory.usage_conn()),
        )));
        ...
        let mut model_catalog = rusty_hand_runtime::model_catalog::ModelCatalog::new();
        ...
        let mut skill_registry = rusty_hand_skills::registry::SkillRegistry::new(skills_dir);
        ...
        let mut extension_registry =
            rusty_hand_extensions::registry::IntegrationRegistry::new(&config.home_dir);
        ...
        let health_config = rusty_hand_extensions::health::HealthMonitorConfig {
        ...
        let extension_health = rusty_hand_extensions::health::HealthMonitor::new(health_config);
        ...
        let web_cache = Arc::new(rusty_hand_runtime::web_cache::WebCache::new(cache_ttl));
        let web_ctx = rusty_hand_runtime::web_search::WebToolsContext {
            search: rusty_hand_runtime::web_search::WebSearchEngine::new(
                config.web.clone(),
                web_cache.clone(),
            ),
        ...
        let embedding_driver: Option<
            Arc<dyn rusty_hand_runtime::embedding::EmbeddingDriver + Send + Sync>,
        > = {
            use rusty_hand_runtime::embedding::create_embedding_driver;
            ...
            } else if std::env::var("VOYAGE_API_KEY").is_ok() {
                // Voyage AI: high-quality embeddings, auto-detect
                match create_embedding_driver("voyage", "voyage-3-lite", "VOYAGE_API_KEY") {
                    Ok(d) => {
                        info!("Embedding driver auto-detected: Voyage AI (voyage-3-lite)");
                        Some(Arc::from(d))
                    }
                    Err(e) => {
                        warn!(error = %e, "Voyage AI embedding auto-detect failed");
                        None
                    }
                }
        ...
        let browser_ctx = rusty_hand_runtime::browser::BrowserManager::new(config.browser.clone());

        // Initialize media understanding engine
        let media_engine =
            rusty_hand_runtime::media_understanding::MediaEngine::new(config.media.clone());
        let tts_engine = rusty_hand_runtime::tts::TtsEngine::new(config.tts.clone());
        ...
        // Paths for engines that persist before `config` is moved into the struct.
        ...
        let kernel = Self {
            ...
            a2a_task_store: rusty_hand_runtime::a2a::A2aTaskStore::default(),
            ...
            auto_reply_engine,
            hooks: rusty_hand_runtime::hooks::HookRegistry::new(),
            process_manager: Arc::new(rusty_hand_runtime::process_manager::ProcessManager::new(5)),
            peer_registry: std::sync::RwLock::new(None),
        ...
        // Seed demo resources if we're in demo mode and the registry is
        // empty (see crate::demo_seed for details and the marker-file
        // contract that prevents re-seeding).
        crate::demo_seed::seed_if_demo_mode(&kernel);
    ...
    pub fn spawn_agent_with_parent(
        ...
        manifest.group =
            normalize_agent_group(manifest.group.take()).map_err(KernelError::RustyHand)?;

        // Overlay kernel default_model onto agent if no custom key/url is set.
        // This ensures agents respect the user's configured provider from `rustyhand init`.
        ...
        self.audit_log.record(
            agent_id.to_string(),
            rusty_hand_runtime::audit::AuditAction::AgentSpawn,
    ...
    pub fn verify_signed_manifest(&self, signed_json: &str) -> KernelResult<String> {
        let signed: rusty_hand_types::manifest_signing::SignedManifest =
            serde_json::from_str(signed_json).map_err(|e| {
                KernelError::RustyHand(rusty_hand_types::error::RustyHandError::Config(format!(
    ...
    pub async fn send_message_with_handle(
        ...
        match result {
            Ok(result) => {
                ...
                self.audit_log.record(
                    agent_id.to_string(),
                    rusty_hand_runtime::audit::AuditAction::AgentMessage,
                    format!(
    ...
    pub fn send_message_streaming(
                    ...
                    Ok(result) => {
                        ...
                        let _ = tx
                            .send(StreamEvent::ContentComplete {
                                stop_reason: rusty_hand_types::message::StopReason::EndTurn,
        ...
        let mut session = self
            ...
            .map_err(KernelError::RustyHand)?
            .unwrap_or_else(|| rusty_hand_memory::session::Session {
                id: entry.session_id,
                agent_id,
                messages: Vec::new(),
        ...
        let needs_compact = {
            use rusty_hand_runtime::compactor::{
        ...
        // Build the structured system prompt via prompt_builder
        {
            ...
            let prompt_ctx = rusty_hand_runtime::prompt_builder::PromptContext {
                ...
                workspace_context: manifest.workspace.as_ref().map(|w| {
                    let mut ws_ctx =
                        rusty_hand_runtime::workspace_context::WorkspaceContext::detect(w);
            ...
            manifest.model.system_prompt =
                rusty_hand_runtime::prompt_builder::build_system_prompt(&prompt_ctx);
        }

        let memory = Arc::clone(&self.memory);
        // Build link context from user message (auto-extract URLs for the agent)
        let message_owned = if let Some(link_ctx) =
            rusty_hand_runtime::link_understanding::build_link_context(message, &self.config.links)
        {
        ...
        let handle = tokio::spawn(async move {
            ...
            let phase_cb: rusty_hand_runtime::agent_loop::PhaseCallback =
                std::sync::Arc::new(move |phase| {
                    use rusty_hand_runtime::agent_loop::LoopPhase;
            ...
            let result = run_agent_loop_streaming(
                ...
                Some(&kernel_clone.config.exec_policy),
            )
            .await;
    ...
    async fn execute_wasm_agent(
        ...
        Ok(AgentLoopResult {
            response,
            total_usage: rusty_hand_types::message::TokenUsage {
                input_tokens: 0,
                output_tokens: 0,
            },
            iterations: 1,
    ...
    async fn execute_llm_agent(
        ...
        let is_stable = self.config.mode == rusty_hand_types::config::KernelMode::Stable;
        ...
        if is_stable {
            ...
            // Resolve aliases (e.g. "sonnet" -> "claude-sonnet-4-6") before scoring
            ...
            let probe = CompletionRequest {
                ...
                messages: vec![rusty_hand_types::message::Message::user(message)],
                tools: tools.clone(),
        ...
        match self.config.usage_footer {
            rusty_hand_types::config::UsageFooterMode::Off => {
            ...
            rusty_hand_types::config::UsageFooterMode::Tokens => {
    ...
    /// Execute a cron job immediately using the same code path as the background scheduler.
    pub async fn execute_cron_job(
        &self,
        job: &rusty_hand_types::scheduler::CronJob,
        ...
        match &job.action {
            rusty_hand_types::scheduler::CronAction::SystemEvent { text } => {
                ...
                let event_type = format!("cron.{}", job_name);
                ...
                Ok(CronRunOutcome::SystemEvent { event_type })
            }
            rusty_hand_types::scheduler::CronAction::AgentTurn {
                ...
                match tokio::time::timeout(timeout, self.send_message(agent_id, message)).await {
                    Ok(Ok(result)) => {
                        tracing::info!(job = %job_name, "Cron job completed successfully");
                        self.cron_scheduler.record_success(job_id);
                        cron_deliver_response(self, agent_id, &result.response, &delivery).await;
                        Ok(CronRunOutcome::AgentTurn {
                            agent_id,
                            response: result.response,
                        })
                    }
                    Ok(Err(e)) => {
                        ...
                        let transient = crate::cron::CronScheduler::is_transient_error(&err_msg);
                        if transient {
                            tracing::warn!(job = %job_name, error = %err_msg, "Cron job failed (transient — will retry at next interval)");
                        } else {
                            tracing::warn!(job = %job_name, error = %err_msg, "Cron job failed (permanent)");
                        }
                        self.cron_scheduler
                            .record_failure(job_id, &err_msg, transient);
    ...
    pub fn switch_agent_session(
        ...
        let session = self
            ...
            .map_err(KernelError::RustyHand)?
            .ok_or_else(|| {
                KernelError::RustyHand(RustyHandError::Internal("Session not found".to_string()))
            })?;

        if session.agent_id != agent_id {
            return Err(KernelError::RustyHand(RustyHandError::Internal(
    ...
    fn save_session_summary(
        ...
        agent_id: AgentId,
        entry: &AgentEntry,
        session: &rusty_hand_memory::session::Session,
    ) {
        use rusty_hand_types::message::{MessageContent, Role};
        ...
        if !servers.is_empty() {
            if let Ok(mcp_tools) = self.mcp_tools.lock() {
                ...
                for tool in mcp_tools.iter() {
                    if let Some(s) = rusty_hand_runtime::mcp::extract_mcp_server(&tool.name) {
                        known_servers.insert(s.to_string());
                    }
                }
                for name in &servers {
                    let normalized = rusty_hand_runtime::mcp::normalize_name(name);
        ...
        let (input_tokens, output_tokens) = session
            .map(|s| {
                ...
                for msg in &s.messages {
                    ...
                    let tokens = len / 4;
                    match msg.role {
                        rusty_hand_types::message::Role::User => input += tokens,
                        rusty_hand_types::message::Role::Assistant => output += tokens,
                        rusty_hand_types::message::Role::System => input += tokens,
                    }
                }
                (input, output)
            })
    ...
    pub async fn compact_agent_session(&self, agent_id: AgentId) -> KernelResult<String> {
        use rusty_hand_runtime::compactor::{compact_session, needs_compaction, CompactionConfig};
        ...
        let result = compact_session(driver, &model, &session, &config)
            .await
            .map_err(|e| KernelError::RustyHand(RustyHandError::Internal(e)))?;
        ...
        let (repaired_messages, repair_stats) =
            rusty_hand_runtime::session_repair::validate_and_repair_with_stats(
    ...
    pub fn context_report(
        &self,
        agent_id: AgentId,
    ) -> KernelResult<rusty_hand_runtime::compactor::ContextReport> {
        use rusty_hand_runtime::compactor::generate_context_report;
        use rusty_hand_runtime::tool_runner::builtin_tool_definitions;
    ...
    pub fn list_bindings(&self) -> Vec<rusty_hand_types::config::AgentBinding> {
        self.bindings
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .clone()
    ...
    pub fn add_binding(&self, binding: rusty_hand_types::config::AgentBinding) {
    ...
    pub fn remove_binding(&self, index: usize) -> Option<rusty_hand_types::config::AgentBinding> {
        let mut bindings = self.bindings.lock().unwrap_or_else(|e| e.into_inner());
    ...
    /// Get the effective runtime budget.
    pub fn budget_config(&self) -> rusty_hand_types::config::BudgetConfig {
        self.runtime_budget
            .read()
            .unwrap_or_else(|e| e.into_inner())
            .clone()
    ...
    /// Update runtime budget values. Values set to `None` are left unchanged.
    ...
    ) -> rusty_hand_types::config::BudgetConfig {
        let mut budget = self
            .runtime_budget
            .write()
            .unwrap_or_else(|e| e.into_inner());
        if let Some(v) = max_hourly_usd {
            budget.max_hourly_usd = v;
        }
        if let Some(v) = max_daily_usd {
            budget.max_daily_usd = v;
        }
        if let Some(v) = max_monthly_usd {
    ...
    /// Snapshot current peer registry if RHP node is running.
    pub fn peer_registry(&self) -> Option<rusty_hand_wire::PeerRegistry> {
        self.peer_registry
            .read()
            .unwrap_or_else(|e| e.into_inner())
            .clone()
    ...
    /// Snapshot current peer node if RHP node is running.
    pub fn peer_node(&self) -> Option<Arc<rusty_hand_wire::PeerNode>> {
        self.peer_node
            .read()
            .unwrap_or_else(|e| e.into_inner())
            .clone()
    ...
    fn apply_hot_actions(
        ...
        new_config: &rusty_hand_types::config::KernelConfig,
    ) {
    ...
    pub fn register_trigger(
        ...
        if self.registry.get(agent_id).is_none() {
            return Err(KernelError::RustyHand(RustyHandError::AgentNotFound(
                agent_id.to_string(),
            )));
        }
        Ok(self
    ...
    pub async fn run_workflow(
        ...
        let run_id = self
            ...
            .await
            .ok_or_else(|| {
                KernelError::RustyHand(RustyHandError::Internal("Workflow not found".to_string()))
            })?;
    ...
    pub fn start_background_agents(self: &Arc<Self>) {
        ...
        // Start RHP peer node if network is enabled
        ...
        // Probe local providers for reachability and model discovery
        {
            ...
            tokio::spawn(async move {
                ...
                for (provider_id, base_url) in &local_providers {
                    let result =
                        rusty_hand_runtime::provider_health::probe_provider(provider_id, base_url)
                            .await;
                    if result.reachable {
                        info!(
        ...
        if self.config.network_enabled {
            info!("RHP network enabled — peer discovery will use shared_secret from config");
        }
        ...
        if let Some(ref a2a_config) = self.config.a2a {
            if a2a_config.enabled && !a2a_config.external_agents.is_empty() {
                ...
                tokio::spawn(async move {
                    let discovered =
                        rusty_hand_runtime::a2a::discover_external_agents(&agents).await;
    ...
    /// Start the RHP peer networking node.
    ///
    ...
    async fn start_ofp_node(self: &Arc<Self>) {
        use rusty_hand_wire::{PeerConfig, PeerNode, PeerRegistry};
        ...
        let node_name = gethostname().unwrap_or_else(|| "rusty-hand-node".to_string());
        ...
        let handle: Arc<dyn rusty_hand_wire::peer::PeerHandle> = self.self_arc();
        ...
        match PeerNode::start(peer_config, registry.clone(), handle.clone()).await {
            Ok((node, _accept_task)) => {
                ...
                info!(
                    node_id = %node_id,
                    listen = %addr,
                    "RHP peer node started"
                );

                *self
                    .peer_registry
                    .write()
                    .unwrap_or_else(|e| e.into_inner()) = Some(registry.clone());
                *self.peer_node.write().unwrap_or_else(|e| e.into_inner()) = Some(node.clone());
                ...
                for peer_addr_str in &self.config.network.bootstrap_peers {
                    ...
                    if let Some(addr) = peer_addr {
                        match node.connect_to_peer(addr, handle.clone()).await {
                            Ok(()) => {
                                info!(peer = %addr, "RHP: connected to bootstrap peer");
                            }
                            Err(e) => {
                                warn!(peer = %addr, error = %e, "RHP: failed to connect to bootstrap peer");
                            }
                        }
                    } else {
                        warn!(addr = %peer_addr_str, "RHP: invalid bootstrap peer address");
                    }
                }
            }
            Err(e) => {
                warn!(error = %e, "RHP: failed to start peer node");
            }
        }
    }
    ...
    fn resolve_driver(&self, manifest: &AgentManifest) -> KernelResult<Arc<dyn LlmDriver>> {
        ...
        if !manifest.fallback_models.is_empty() {
            ...
            if chain.len() > 1 {
                return Ok(Arc::new(
                    rusty_hand_runtime::drivers::fallback::FallbackDriver::new(chain),
                ));
            }
        }
    ...
    async fn connect_mcp_servers(self: &Arc<Self>) {
        use rusty_hand_runtime::mcp::{McpConnection, McpServerConfig, McpTransport};
        use rusty_hand_types::config::McpTransportEntry;
        ...
        if let Ok(mcp_tools) = self.mcp_tools.lock() {
            if mcp_allowlist.is_empty() {
                ...
                let normalized: Vec<String> = mcp_allowlist
                    .iter()
                    .map(|s| rusty_hand_runtime::mcp::normalize_name(s))
                    .collect();
                all_tools.extend(
                    mcp_tools
                        .iter()
                        .filter(|t| {
                            rusty_hand_runtime::mcp::extract_mcp_server(&t.name)
    ...
    pub fn reload_skills(&self) {
        ...
        let mut fresh = rusty_hand_skills::registry::SkillRegistry::new(skills_dir);
...
async fn cron_deliver_response(
    ...
    agent_id: AgentId,
    response: &str,
    delivery: &rusty_hand_types::scheduler::CronDelivery,
) {
    use rusty_hand_types::scheduler::CronDelivery;
    ...
    match delivery {
        ...
        CronDelivery::Channel { channel, to } => {
            ...
            let _ = kernel
                .memory
                .structured_set(agent_id, "delivery.last_channel", kv_val.clone());
            // Deliver via channel API
            cron_send_to_channel(kernel, channel, to, response).await;
        }
                ...
                .structured_get(agent_id, "delivery.last_channel")
            {
                Ok(Some(val)) => {
                    ...
                    if !channel.is_empty() && !recipient.is_empty() {
                        cron_send_to_channel(kernel, channel, recipient, response).await;
                    }
                }
                _ => {
...
async fn cron_send_to_channel(
        ...
        .build()
    {
        Ok(c) => c,
        Err(e) => {
            tracing::warn!(error = %e, "Cron channel delivery: failed to build HTTP client");
            return;
        }
    };

    // UTF-8 aware chunking helper (doesn't split multi-byte chars)
    fn utf8_chunks(s: &str, max_bytes: usize) -> Vec<&str> {
    ...
    match channel {
        "telegram" => {
            // SECURITY: validate recipient is a numeric chat_id (positive for users,
            // negative for groups/channels) to prevent injection into the API URL
            let is_valid_chat_id = {
                let r = recipient.strip_prefix('-').unwrap_or(recipient);
                !r.is_empty() && r.chars().all(|c| c.is_ascii_digit())
            };
            if !is_valid_chat_id {
                tracing::warn!(
                    recipient = %recipient,
                    "Cron: invalid Telegram chat_id (must be numeric, optionally negative)"
                );
                return;
            }
            ...
            if let Some(token) = token {
                ...
                tracing::warn!("Cron: Telegram token not available for channel delivery");
            }
        }
        "slack" => {
        ...
        _ => {
            tracing::warn!(channel = %channel, "Cron: unsupported channel for native delivery");
        }
    }
}

/// Parsed fields from a cron job JSON payload (shared by create and update).
struct CronJobFields {
    name: String,
    schedule: rusty_hand_types::scheduler::CronSchedule,
    action: rusty_hand_types::scheduler::CronAction,
    delivery: rusty_hand_types::scheduler::CronDelivery,
    one_shot: bool,
    enabled: bool,
}

/// Parse the common fields from a cron job JSON payload.
fn parse_cron_job_fields(job_json: &serde_json::Value) -> Result<CronJobFields, String> {
    use rusty_hand_types::scheduler::{CronAction, CronDelivery, CronSchedule};
...
#[async_trait]
impl KernelHandle for RustyHandKernel {
    async fn spawn_agent(
        ...
        let content_hash = rusty_hand_types::manifest_signing::hash_manifest(manifest_toml);
    ...
    fn kill_agent(&self, agent_id: &str) -> Result<(), String> {
        ...
        RustyHandKernel::kill_agent(self, id).map_err(|e| format!("Kill failed: {e}"))
    }
    ...
    async fn doc_ingest(
        ...
        let embed_driver = self
            .embedding_driver
            .as_ref()
            .ok_or("No embedding driver configured — set memory.embedding_provider in config")?;
        ...
        for (i, chunk) in chunks.iter().enumerate() {
            ...
            self.memory
                .remember_with_embedding(
                    id,
                    chunk,
                    rusty_hand_types::memory::MemorySource::Document,
                    "document",
                    metadata,
                    Some(&embedding),
                )
                .map_err(|e| format!("Store failed for chunk {i}: {e}"))?;
            stored += 1;
        }
    ...
    async fn doc_search(
        ...
        let embed_driver = self
            .embedding_driver
            .as_ref()
            .ok_or("No embedding driver configured")?;

        let query_embedding = embed_driver
            .embed_one(query)
            .await
        ...
        let results = self
            .memory
            .recall_with_embedding(query, limit, Some(filter), Some(&query_embedding))
            .map_err(|e| format!("Search failed: {e}"))?;

        let json_results: Vec<serde_json::Value> = results
            .into_iter()
            .map(|m| {
                // Compute score against query embedding
                let score = m
                    .embedding
                    .as_deref()
                    .map(|e| rusty_hand_runtime::embedding::cosine_similarity(&query_embedding, e))
                    .unwrap_or(0.0);
                serde_json::json!({
                    "content": m.content,
                    "score": score,
                    "source": m.metadata.get("source").and_then(|v| v.as_str()).unwrap_or("unknown"),
                    "chunk_index": m.metadata.get("chunk_index").and_then(|v| v.as_u64()).unwrap_or(0),
                })
            })
            .collect();
    ...
    fn agent_metrics(&self, agent_id: &str) -> Result<serde_json::Value, String> {
        let id: AgentId = agent_id
            .parse()
            .map_err(|_| "Invalid agent ID".to_string())?;

        // Usage from metering engine
        let summary = self
            .metering
            .get_summary(Some(id))
        ...
        let uptime_secs = agent
            .as_ref()
            .map(|a| (chrono::Utc::now() - a.created_at).num_seconds().max(0) as u64)
            .unwrap_or(0);
        let last_active = agent
            .as_ref()
            .map(|a| a.last_active.to_rfc3339())
            .unwrap_or_default();
    ...
    fn update_agent_system_prompt(&self, agent_id: &str, new_prompt: &str) -> Result<(), String> {
        let id: AgentId = agent_id
            ...
            .map_err(|_| "Invalid agent ID".to_string())?;
        self.registry
            .update_system_prompt(id, new_prompt.to_string())
            .map_err(|e| format!("Prompt update failed: {e}"))
    }
        ...
        self.registry
            ...
            .filter(|e| {
                ...
                let group_match = e
                    ...
                    .group
                    .as_deref()
                    .map(|group| group.to_lowercase().contains(&q))
                    .unwrap_or(false);
                name_match || tag_match || tool_match || desc_match || group_match
            })
            .map(|e| kernel_handle::AgentInfo {
                id: e.id.to_string(),
                name: e.name.clone(),
    ...
    async fn publish_event(
        ...
        RustyHandKernel::publish_event(self, event).await;
        Ok(())
    }

    async fn knowledge_add_entity(
        &self,
        entity: rusty_hand_types::memory::Entity,
    ) -> Result<String, String> {
        self.memory
            .add_entity(entity)
            .await
    ...
    async fn knowledge_add_relation(
        &self,
        relation: rusty_hand_types::memory::Relation,
    ) -> Result<String, String> {
        self.memory
    ...
    async fn cron_create(
        ...
        use rusty_hand_types::scheduler::{CronJob, CronJobId};

        let fields = parse_cron_job_fields(&job_json)?;
    ...
    /// Update an existing cron job, preserving its ID and history.
    async fn cron_update(
        ...
        let id =
            CronJobId(uuid::Uuid::parse_str(job_id).map_err(|e| format!("Invalid job ID: {e}"))?);
        let fields = parse_cron_job_fields(&job_json)?;
        let agent_id = job_json["agent_id"]
            .as_str()
        ...
        let job = CronJob {
            ...
            created_at: chrono::Utc::now(), // preserved by update_job
            next_run: None,                 // recomputed by update_job
            last_run: None,                 // preserved by update_job
        };

        self.cron_scheduler
            .update_job(id, job, fields.one_shot)
            .map_err(|e| format!("{e}"))?;
    ...
    async fn cron_list_all(&self) -> Result<Vec<serde_json::Value>, String> {
        let jobs = self.cron_scheduler.list_all_jobs();
        let json_jobs: Vec<serde_json::Value> = jobs
            .into_iter()
            .map(|j| serde_json::to_value(&j).unwrap_or_default())
            .collect();
    ...
    async fn cron_cancel(&self, job_id: &str) -> Result<(), String> {
        let id = rusty_hand_types::scheduler::CronJobId(
    ...
    async fn request_approval(
        ...
        use rusty_hand_types::approval::{ApprovalDecision, ApprovalRequest as TypedRequest};
    ...
    async fn spawn_agent_checked(
        ...
        manifest_toml: &str,
        parent_id: Option<&str>,
        parent_caps: &[rusty_hand_types::capability::Capability],
    ) -> Result<(String, String), String> {
        ...
        rusty_hand_types::capability::validate_capability_inheritance(parent_caps, &child_caps)?;
...
#[async_trait]
impl rusty_hand_wire::peer::PeerHandle for RustyHandKernel {
    fn local_agents(&self) -> Vec<rusty_hand_wire::message::RemoteAgentInfo> {
        self.registry
            .list()
            .iter()
            .map(|entry| rusty_hand_wire::message::RemoteAgentInfo {
    ...
    fn discover_agents(&self, query: &str) -> Vec<rusty_hand_wire::message::RemoteAgentInfo> {
        let q = query.to_lowercase();
        self.registry
            .list()
            .iter()
...
mod tests {
    ...
    #[test]
    fn test_truncate_utf8_bytes_respects_char_boundaries() {
        let s = "🙂abc";
        assert_eq!(truncate_utf8_bytes(s, 0), "");
        assert_eq!(truncate_utf8_bytes(s, 1), "");
        assert_eq!(truncate_utf8_bytes(s, 4), "🙂");
        assert_eq!(truncate_utf8_bytes(s, 5), "🙂a");
        assert_eq!(truncate_utf8_bytes(s, 128), s);
    }
    ...
    #[test]
    fn test_delivery_tracker_global_eviction_prefers_oldest() {
        let tracker = DeliveryTracker::new();
        let old_agent = AgentId::new();
        let new_agent = AgentId::new();

        let old_ts = chrono::Utc::now() - chrono::Duration::minutes(10);
        let new_ts = chrono::Utc::now();

        // Preload internal storage close to the global cap.
        tracker.receipts.insert(
            old_agent,
            vec![receipt_with_ts(old_ts); DeliveryTracker::MAX_RECEIPTS],
        );
        tracker
            .receipts
            .insert(new_agent, vec![receipt_with_ts(new_ts)]);

        // Trigger eviction path.
        tracker.record(new_agent, receipt_with_ts(new_ts));
        ...
        // We inserted 2 records over cap, so the oldest agent should lose at least 2.
