...
//! to RustyHand users with roles, then enforces permission checks on actions.

use argon2::password_hash::{PasswordHash, PasswordVerifier};
use argon2::Argon2;
use dashmap::DashMap;
use rusty_hand_types::agent::UserId;
use rusty_hand_types::config::UserConfig;
...
pub enum Action {
    /// Read-only dashboard/API access.
    ReadOnly,
    /// Chat with an agent.
    ChatWithAgent,
    /// Spawn a new agent.
    ...
    ManageUsers,
    /// Export or restore persistent state.
    ManageBackups,
}
...
impl Action {
    ...
    pub fn required_role(&self) -> UserRole {
        match self {
            Action::ReadOnly => UserRole::Viewer,
            ...
            Action::ManageBackups => UserRole::Owner,
        }
    }
}
...
pub struct AuthManager {
    /// Known users by their RustyHand user ID.
    ...
    /// API key hash index: UserId → password hash / sha256 digest.
    api_key_hashes: DashMap<UserId, String>,
}
...
impl AuthManager {
    ...
    pub fn new(user_configs: &[UserConfig]) -> Self {
        ...
        for config in user_configs {
            ...
            if let Some(api_key_hash) = config
                .api_key_hash
                .as_ref()
                .filter(|h| !h.trim().is_empty())
            {
                manager
                    .api_key_hashes
                    .insert(user_id, api_key_hash.trim().to_string());
            }
    ...
    ///
    /// Returns the RustyHand UserId if a matching channel binding exists,
    ...
    /// Authenticate a user from a configured per-user API key.
    pub fn authenticate_api_key(&self, api_key: &str) -> Option<UserIdentity> {
        for entry in &self.api_key_hashes {
            if verify_api_key_hash(entry.value(), api_key) {
                return self.get_user(*entry.key());
            }
        }
        None
    }
    ...
    pub fn authorize(&self, user_id: UserId, action: &Action) -> RustyHandResult<()> {
        let identity = self
            .users
            .get(&user_id)
            .ok_or_else(|| RustyHandError::AuthDenied("Unknown user".to_string()))?;

        let required = action.required_role();
        if identity.role >= required {
            Ok(())
        } else {
            Err(RustyHandError::AuthDenied(format!(
    ...
    /// Returns true when any user-level API key is configured.
...
fn verify_api_key_hash(stored_hash: &str, api_key: &str) -> bool {
    ...
    if digest.len() != stored_hash.len() {
        return false;
    }
    digest
        .as_bytes()
        .ct_eq(stored_hash.to_ascii_lowercase().as_bytes())
        .into()
}
...
mod tests {
    ...
    fn test_configs() -> Vec<UserConfig> {
        let guest_hash = Argon2::default()
            .hash_password(b"guest-key", &SaltString::generate(&mut OsRng))
            .unwrap()
            .to_string();
        vec![
            UserConfig {
                ...
                api_key_hash: Some(hex::encode(Sha256::digest(b"owner-key"))),
            },
            UserConfig {
    ...
    #[test]
    fn test_authenticate_per_user_api_key() {
        let manager = AuthManager::new(&test_configs());

        let owner = manager.authenticate_api_key("owner-key").unwrap();
        assert_eq!(owner.name, "Alice");
        assert_eq!(owner.role, UserRole::Owner);

        let guest = manager.authenticate_api_key("guest-key").unwrap();
        assert_eq!(guest.name, "Guest");
        assert_eq!(guest.role, UserRole::User);

        assert!(manager.authenticate_api_key("wrong-key").is_none());
    }
    ...
    fn test_has_api_keys() {
        ...
        assert!(!AuthManager::new(&[]).has_api_keys());
    }
