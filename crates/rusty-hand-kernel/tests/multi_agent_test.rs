...
fn test_config() -> KernelConfig {
    let tmp = std::env::temp_dir().join("rusty-hand-multi-agent-test");
...
async fn test_six_agent_fleet() {
    ...
    let kernel = RustyHandKernel::boot_with_config(test_config()).expect("Kernel should boot");
...
"#,
    ...
    println!("\n{}", "=".repeat(60));
    println!("  RUSTY_HAND MULTI-AGENT FLEET TEST");
