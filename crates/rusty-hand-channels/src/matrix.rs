...
pub struct MatrixAdapter {
    ...
    /// Bot's user ID (e.g., "@rustyhand:matrix.org").
    user_id: String,
...
impl MatrixAdapter {
    ...
    fn is_allowed_room(&self, room_id: &str) -> bool {
        crate::types::is_in_allowlist(&self.allowed_rooms, room_id)
    }
}
...
impl ChannelAdapter for MatrixAdapter {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = ChannelMessage> + Send>>, Box<dyn std::error::Error>>
    {
        ...
        // SECURITY: warn if allowed_rooms is empty (bot accepts messages from all joined rooms)
        if self.allowed_rooms.is_empty() {
            warn!(
                "Matrix adapter has no allowed_rooms configured — \
                 messages from all joined rooms will be accepted. \
                 Set [channels.matrix].allowed_rooms in config to restrict access."
            );
        }
