...
use crate::types::{
    is_in_allowlist, split_message, ChannelAdapter, ChannelContent, ChannelMessage, ChannelType,
    ChannelUser,
};
...
impl TwistAdapter {
    ...
    fn is_allowed_channel(&self, channel_id: &str) -> bool {
        is_in_allowlist(&self.allowed_channels, channel_id)
    }
}
...
impl ChannelAdapter for TwistAdapter {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = ChannelMessage> + Send>>, Box<dyn std::error::Error>>
    {
        ...
        // SECURITY: warn if allowed_channels is empty (bot accepts messages from all channels)
        if self.allowed_channels.is_empty() {
            warn!(
                "Twist adapter has no allowed_channels configured — \
                 messages from all workspace channels will be accepted. \
                 Set [channels.twist].allowed_channels in config to restrict access."
            );
        }
