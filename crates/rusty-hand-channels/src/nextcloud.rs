...
impl NextcloudAdapter {
    ...
    fn is_allowed_room(&self, room_token: &str) -> bool {
        crate::types::is_in_allowlist(&self.allowed_rooms, room_token)
    }
}
...
impl ChannelAdapter for NextcloudAdapter {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = ChannelMessage> + Send>>, Box<dyn std::error::Error>>
    {
        ...
        // SECURITY: warn if allowed_rooms is empty (bot accepts messages from all rooms)
        if self.allowed_rooms.is_empty() {
            warn!(
                "Nextcloud Talk adapter has no allowed_rooms configured — \
                 messages from all rooms will be accepted. \
                 Set [channels.nextcloud_talk].allowed_rooms in config to restrict access."
            );
        }
