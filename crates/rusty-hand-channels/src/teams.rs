//! Microsoft Teams channel adapter for the RustyHand channel bridge.
//!
...
impl TeamsAdapter {
    ...
    fn is_allowed_tenant(&self, tenant_id: &str) -> bool {
        crate::types::is_in_allowlist(&self.allowed_tenants, tenant_id)
    }
}
...
impl ChannelAdapter for TeamsAdapter {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = ChannelMessage> + Send>>, Box<dyn std::error::Error>>
    {
        ...
        tokio::spawn(async move {
            ...
            // SECURITY: warn if allowed_tenants is empty (bot accepts messages from any tenant)
            if tenants_shared.is_empty() {
                warn!(
                    "Teams adapter has no allowed_tenants configured — \
                     messages from any Azure AD tenant will be accepted. \
                     Set [channels.teams].allowed_tenants in config to restrict access."
                );
            }
