//! Mattermost channel adapter for the RustyHand channel bridge.
//!
...
impl MattermostAdapter {
    ...
    fn is_allowed_channel(&self, channel_id: &str) -> bool {
        crate::types::is_in_allowlist(&self.allowed_channels, channel_id)
    }
}
        ...
        *self.bot_user_id.write().await = Some(user_id);
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // SECURITY: warn once if allowed_channels is empty
                if allowed_channels.is_empty() {
                    warn!(
                        "Mattermost bot has no allowed_channels configured — \
                         messages from any channel will be accepted. \
                         Set [channels.mattermost].allowed_channels in config to restrict access."
                    );
                }
                ...
                let should_reconnect = 'inner: loop {
                    ...
                    {
                        debug!(
                            "Mattermost message from channel={}: {:?}",
                            channel_msg.sender.platform_id, channel_msg.content
                        );
                        drop(bot_id_guard);
