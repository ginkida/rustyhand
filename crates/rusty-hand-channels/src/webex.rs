    ...
    ) -> Result<Pin<Box<dyn Stream<Item = ChannelMessage> + Send>>, Box<dyn std::error::Error>>
    {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // SECURITY: warn once if allowed_rooms is empty
                if allowed_rooms.is_empty() {
                    warn!(
                        "Webex adapter has no allowed_rooms configured — \
                         messages from all rooms will be accepted. \
                         Set [channels.webex].allowed_rooms in config to restrict access."
                    );
                }
