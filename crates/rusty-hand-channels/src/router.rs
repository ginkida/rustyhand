...
use rusty_hand_types::config::{AgentBinding, BroadcastConfig, BroadcastStrategy};
use std::sync::Mutex;
use tracing::warn;
...
pub struct AgentRouter {
    /// Default agent per user (keyed by rusty_hand_user or platform_id).
...
mod tests {
    ...
    fn test_binding_channel_match() {
        ...
        router.load_bindings(&[AgentBinding {
            ...
            match_rule: rusty_hand_types::config::BindingMatchRule {
    ...
    fn test_binding_specificity_scores() {
        use rusty_hand_types::config::BindingMatchRule;
