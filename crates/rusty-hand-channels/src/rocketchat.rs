...
impl ChannelAdapter for RocketChatAdapter {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = ChannelMessage> + Send>>, Box<dyn std::error::Error>>
    {
        ...
        if self.allowed_channels.is_empty() {
            warn!(
                "Rocket.Chat adapter has no allowed_channels configured — \
                 messages from all channels will be accepted. \
                 Set [channels.rocketchat].allowed_channels in config to restrict access."
            );
        }
