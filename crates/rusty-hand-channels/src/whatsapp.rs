...
impl ChannelAdapter for WhatsAppAdapter {
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = ChannelMessage> + Send>>, Box<dyn std::error::Error>>
    {
        ...
        // SECURITY: warn if allowed_users is empty (bot accepts messages from any phone number)
        if self.allowed_users.is_empty() {
            warn!(
                "WhatsApp adapter has no allowed_users configured — \
                 messages from any phone number will be accepted. \
                 Set [channels.whatsapp].allowed_users in config to restrict access."
            );
        }
