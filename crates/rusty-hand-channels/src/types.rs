...
pub struct ChannelUser {
    ...
    /// Optional mapping to an RustyHand user identity.
    pub rusty_hand_user: Option<String>,
}
...
// Re-export policy/format types from rusty-hand-types for convenience.
pub use rusty_hand_types::config::{DmPolicy, GroupPolicy, OutputFormat};

/// Trait that every channel adapter must implement.
///
/// A channel adapter bridges a messaging platform to the RustyHand kernel by converting
...
/// Check whether `value` is in `list`, treating an empty list as "allow all".
///
/// Used by channel adapters to filter by allowed room/channel/team/sender IDs.
pub fn is_in_allowlist(list: &[impl AsRef<str>], value: &str) -> bool {
    list.is_empty() || list.iter().any(|x| x.as_ref() == value)
}
...
mod tests {
    ...
    fn test_agent_phase_tool_use_sanitizes() {
        ...
        if let AgentPhase::ToolUse { tool_name } = phase {
            ...
            unreachable!("Expected ToolUse variant");
        }
    }
