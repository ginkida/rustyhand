//! IRC channel adapter for the RustyHand channel bridge.
//!
...
//! PRIVMSG, PING/PONG. TLS support requires adding `tokio-native-tls` and is
//! not yet implemented.
...
pub struct IrcAdapter {
    ...
    /// IRC channels to join (e.g., ["#rustyhand", "#bots"]).
    channels: Vec<String>,
    /// Shutdown signal.
        ...
        *self.write_tx.write().await = Some(write_cmd_tx.clone());
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                registration.push_str(&format!("USER {nick} 0 * :RustyHand Bot\r\n"));
                    ...
                    tokio::select! {
                        ...
                        _ = shutdown_rx.changed() => {
                            if *shutdown_rx.borrow() {
                                ...
                                let _ = writer.write_all(b"QUIT :RustyHand shutting down\r\n").await;
                                return;
                            }
                        }
                    }
...
mod tests {
    ...
    fn test_parse_irc_line_privmsg() {
        let line = parse_irc_line(":alice!alice@host PRIVMSG #rustyhand :Hello everyone!").unwrap();
        assert_eq!(line.prefix.as_deref(), Some("alice!alice@host"));
        assert_eq!(line.command, "PRIVMSG");
        assert_eq!(line.params, vec!["#rustyhand"]);
    ...
    fn test_parse_irc_line_no_trailing() {
        let line = parse_irc_line(":alice!alice@host JOIN #rustyhand").unwrap();
    ...
    fn test_parse_privmsg_channel() {
        let line = IrcLine {
            ...
            params: vec!["#rustyhand".to_string()],
            trailing: Some("Hello from IRC!".to_string()),
        };

        let msg = parse_privmsg(&line, "rusty-hand-bot").unwrap();
        assert_eq!(msg.channel, ChannelType::Custom("irc".to_string()));
        assert_eq!(msg.sender.display_name, "alice");
        assert_eq!(msg.sender.platform_id, "#rustyhand");
        assert!(msg.is_group);
    ...
    fn test_parse_privmsg_dm() {
        let line = IrcLine {
            ...
            command: "PRIVMSG".to_string(),
            params: vec!["rusty-hand-bot".to_string()],
    ...
    fn test_parse_privmsg_skips_self() {
        let line = IrcLine {
            prefix: Some("rusty-hand-bot!bot@host".to_string()),
            command: "PRIVMSG".to_string(),
            params: vec!["#rustyhand".to_string()],
            trailing: Some("My own message".to_string()),
        };

        let msg = parse_privmsg(&line, "rusty-hand-bot");
        assert!(msg.is_none());
    }
