...
impl SemanticStore {
    ...
    /// Maximum content size for a single memory fragment (256 KB).
    ...
    pub fn remember_with_embedding(
        ...
        // SECURITY: prevent disk exhaustion via unbounded memory writes
    ...
    pub fn recall_with_embedding(
        ...
        for row_result in rows {
            let (
                ...
                access_count,
                embedding_bytes,
            ) = row_result.map_err(|e| RustyHandError::Memory(e.to_string()))?;
    ...
    pub fn forget(&self, id: MemoryId) -> RustyHandResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| RustyHandError::Internal(e.to_string()))?;
