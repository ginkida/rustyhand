//! RustyHand CLI — command-line interface for the RustyHand Agent OS.
//!
//! When a daemon is running (`rustyhand start`), the CLI talks to it over HTTP.
...
use rand::rngs::OsRng;
use rusty_hand_api::server::read_daemon_info;
use rusty_hand_kernel::RustyHandKernel;
use rusty_hand_types::agent::{AgentId, AgentManifest};
...
\x1b[1;36mExamples:\x1b[0m
  rustyhand init                 Initialize config and data directories
  rustyhand start                Start the kernel daemon
  rustyhand tui                  Launch the interactive terminal dashboard
  rustyhand chat                 Quick chat with the default agent
  rustyhand agent new coder      Spawn a new agent from a template
  rustyhand models list          Browse available LLM models
  rustyhand add github           Install the GitHub integration
  rustyhand doctor               Run diagnostic health checks
  rustyhand channel setup        Interactive channel setup wizard
  rustyhand cron list            List scheduled jobs

\x1b[1;36mQuick Start:\x1b[0m
  1. rustyhand init              Set up config + API key
  2. rustyhand start             Launch the daemon
  3. rustyhand chat              Start chatting!

\x1b[1;36mMore:\x1b[0m
  Docs:       https://github.com/ginkida/rustyhand
  Dashboard:  http://127.0.0.1:4200/ (when daemon is running)";

/// RustyHand — the open-source Agent Operating System.
#[derive(Parser)]
#[command(
    name = "rustyhand",
    version,
    about = "\u{1F40D} RustyHand \u{2014} Open-source Agent Operating System",
    long_about = "\u{1F40D} RustyHand \u{2014} Open-source Agent Operating System\n\n\
...
#[derive(Subcommand)]
enum Commands {
    /// Initialize RustyHand (create ~/.rustyhand/ and default config).
    Init {
...
enum ConfigCommands {
    ...
    /// Save an API key to ~/.rustyhand/.env (prompts interactively).
    SetKey {
        /// Provider name (groq, anthropic, openai, gemini, deepseek, etc.).
        provider: String,
    },
    /// Remove an API key from ~/.rustyhand/.env.
    DeleteKey {
        /// Provider name.
        provider: String,
    },
...
enum SecurityCommands {
    ...
    Verify,
    /// Generate an Argon2 hash for [[users]].api_key_hash.
    HashKey {
        /// API key to hash. If omitted, read a single line from stdin.
        key: Option<String>,
    },
}
...
enum MemoryCommands {
    ...
    /// Export a full backup of the persistent memory database.
    Export {
        /// Output file path.
        path: PathBuf,
        /// Backup format (defaults to file extension or json).
        #[arg(long)]
        format: Option<BackupFormatArg>,
    },
    /// Import a previously exported memory backup.
    Import {
        /// Backup file path.
        path: PathBuf,
        /// Backup format (defaults to file extension).
        #[arg(long)]
        format: Option<BackupFormatArg>,
    },
}
...
fn main() {
    // Load ~/.rustyhand/.env into process environment (system env takes priority).
    dotenv::load_dotenv();
    ...
    let is_mcp_mode = matches!(cli.command, Some(Commands::Mcp));

    if is_mcp_mode {
        // MCP uses stdin/stdout for JSON-RPC — tracing must NOT touch stdout.
        init_tracing_file();
    } else if is_tui_mode {
        init_tracing_file();
    } else {
    ...
    match cli.command {
        ...
        Some(Commands::Security(sub)) => match sub {
            ...
            SecurityCommands::HashKey { key } => cmd_security_hash_key(key),
        },
        Some(Commands::Memory(sub)) => match sub {
            ...
            MemoryCommands::Export { path, format } => cmd_memory_export(&path, format),
            MemoryCommands::Import { path, format } => cmd_memory_import(&path, format),
        },
...
pub(crate) fn daemon_default_headers() -> reqwest::header::HeaderMap {
    static API_KEY: OnceLock<Option<String>> = OnceLock::new();

    let api_key = API_KEY
        .get_or_init(|| {
            if let Ok(api_key) = std::env::var("RUSTY_HAND_API_KEY") {
                if !api_key.trim().is_empty() {
                    return Some(api_key);
                }
            }

            let config = rusty_hand_kernel::config::load_config(None);
...
pub(crate) fn daemon_client() -> reqwest::blocking::Client {
    reqwest::blocking::Client::builder()
        .default_headers(daemon_default_headers())
    ...
    match resp {
        Ok(r) => {
            ...
            if status.is_server_error() {
                let log_path = rusty_hand_home().join("tui.log");
                ui::error_with_fix(
                    &format!("Daemon returned error ({})", status),
                    &format!("Check daemon logs: {}", log_path.display()),
                );
            }
            body
        }
        Err(e) => {
            ...
            if msg.contains("timed out") || msg.contains("Timeout") {
                ui::error_with_fix(
                    "Request timed out",
                    "The agent may be processing a complex request. Try again, or check `rustyhand status`",
                );
            } else if msg.contains("Connection refused") || msg.contains("connect") {
                ui::error_with_fix(
                    "Cannot connect to daemon",
                    "Is the daemon running? Start it with: rustyhand start",
                );
            } else {
                ui::error_with_fix(
                    &format!("Daemon communication error: {msg}"),
                    "Check `rustyhand status` or restart: rustyhand start",
                );
            }
            std::process::exit(1);
        }
...
fn cmd_init(quick: bool, refresh: bool) {
    ...
    let base_dir = rusty_hand_dir
        .parent()
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|| rusty_hand_dir.clone());

    // --- Ensure directories exist ---
    if !rusty_hand_dir.exists() {
        std::fs::create_dir_all(&rusty_hand_dir).unwrap_or_else(|e| {
            ui::error_with_fix(
                &format!("Failed to create {}", rusty_hand_dir.display()),
                &format!("Check permissions on {}", base_dir.display()),
            );
            eprintln!("  {e}");
            std::process::exit(1);
        });
        restrict_dir_permissions(&rusty_hand_dir);
    }

    for sub in ["data", "agents"] {
        let dir = rusty_hand_dir.join(sub);
        if !dir.exists() {
    ...
    if quick {
        ...
        cmd_init_interactive(&rusty_hand_dir);
    }
}

/// Quick init: no prompts, auto-detect, write config + .env, print next steps.
fn cmd_init_quick(rusty_hand_dir: &std::path::Path) {
    ui::banner();
    ui::blank();

    let (provider, api_key_env, model) = detect_best_provider();

    write_config_if_missing(rusty_hand_dir, provider, model, api_key_env);

    ui::blank();
    ui::success("RustyHand initialized (quick mode)");
    ui::kv("Provider", provider);
    ui::kv("Model", model);
    ui::blank();
    ui::next_steps(&[
        "Start the daemon:  rustyhand start",
        "Chat:              rustyhand chat",
    ]);
}

/// Interactive 5-step onboarding wizard (ratatui TUI).
fn cmd_init_interactive(rusty_hand_dir: &std::path::Path) {
    ...
    match init_wizard::run() {
        InitResult::Completed {
            ...
            ui::blank();
            ui::success("RustyHand initialized!");
            ui::kv("Provider", &provider);
            ui::kv("Model", &model);
            ...
            match launch {
                LaunchChoice::Desktop => {
                    launch_desktop_app(rusty_hand_dir);
                }
                LaunchChoice::Dashboard => {
                    if let Some(base) = find_daemon() {
                        ...
                        ui::error("Daemon is not running. Start it with: rustyhand start");
                    }
                }
                LaunchChoice::Chat => {
...
/// Launch the rusty-hand-desktop Tauri app, connecting to the running daemon.
fn launch_desktop_app(_rusty_hand_dir: &std::path::Path) {
    ...
    let desktop_bin = {
        ...
        #[cfg(windows)]
        let name = "rusty-hand-desktop.exe";
        #[cfg(not(windows))]
        let name = "rusty-hand-desktop";
    ...
    match desktop_bin {
        Some(ref path) if path.exists() => {
            ui::success("Launching RustyHand Desktop...");
                ...
                .spawn()
            {
                ...
                Err(e) => {
                    ...
                    ui::hint("Try: rustyhand dashboard");
                }
            }
        }
        _ => {
            ui::error("Desktop app not found.");
            ui::hint("Install it with: cargo install rusty-hand-desktop");
...
fn write_config_if_missing(
    rusty_hand_dir: &std::path::Path,
    provider: &str,
    model: &str,
    api_key_env: &str,
) {
    let config_path = rusty_hand_dir.join("config.toml");
    if config_path.exists() {
        ui::check_ok(&format!("Config already exists: {}", config_path.display()));
    } else {
        let default_config = format!(
            r#"# RustyHand Agent OS configuration
# See https://github.com/ginkida/rustyhand for documentation
...
fn cmd_start(config: Option<PathBuf>) {
    if let Some(base) = find_daemon() {
        ui::error_with_fix(
            ...
            "Use `rustyhand status` to check it, or stop it first",
        );
        std::process::exit(1);
    }
    ...
    rt.block_on(async {
        ...
        if let Err(e) =
            rusty_hand_api::server::run_daemon(kernel, &listen_addr, Some(&daemon_info_path)).await
        {
        ...
        ui::blank();
        println!("  RustyHand daemon stopped.");
    });
}
    ...
    match find_daemon() {
        ...
        None => {
            ui::warn_with_fix(
                ...
                "Is it running? Check with: rustyhand status",
            );
        }
    }
}
...
fn boot_kernel_error(e: &rusty_hand_kernel::error::KernelError) {
    let msg = e.to_string();
    if msg.contains("parse") || msg.contains("toml") || msg.contains("config") {
        ui::error_with_fix(
            "Failed to parse configuration",
            "Check your config.toml syntax: rustyhand config show",
        );
    } else if msg.contains("database") || msg.contains("locked") || msg.contains("sqlite") {
        ui::error_with_fix(
            "Database error (file may be locked)",
            "Check if another RustyHand process is running: rustyhand status",
        );
    } else if msg.contains("key") || msg.contains("API") || msg.contains("auth") {
        ui::error_with_fix(
            "LLM provider authentication failed",
            "Run `rustyhand doctor` to check your API key configuration",
        );
    } else {
        ui::error_with_fix(
            &format!("Failed to boot kernel: {msg}"),
            "Run `rustyhand doctor` to diagnose the issue",
        );
    }
}
...
fn cmd_agent_spawn(config: Option<PathBuf>, manifest_path: PathBuf) {
    if !manifest_path.exists() {
        ui::error_with_fix(
            ...
            "Use `rustyhand agent new` to spawn from a template instead",
        );
        std::process::exit(1);
    }
    ...
    if let Some(base) = find_daemon() {
        ...
        match kernel.spawn_agent(manifest) {
            Ok(id) => {
                ...
                println!("  For persistent agents, use `rustyhand start` first.");
            }
            Err(e) => {
...
fn cmd_agent_new(config: Option<PathBuf>, template_name: Option<String>) {
    ...
    if all_templates.is_empty() {
        ui::error_with_fix(
            ...
            "Run `rustyhand init` to set up the agents directory",
        );
        std::process::exit(1);
    }
        ...
        Some(ref name) => match all_templates.iter().find(|t| t.name == *name) {
            ...
            None => {
                ui::error_with_fix(
                    ...
                    "Run `rustyhand agent new` to see available templates",
                );
                std::process::exit(1);
            }
        },
...
fn spawn_template_agent(config: Option<PathBuf>, template: &templates::AgentTemplate) {
    if let Some(base) = find_daemon() {
        ...
        if let Some(id) = body["agent_id"].as_str() {
            ...
            ui::blank();
            ui::hint(&format!("Chat: rustyhand chat {}", template.name));
        } else {
            ui::error(&format!(
                "Failed to spawn: {}",
                body["error"].as_str().unwrap_or("Unknown error")
        ...
        match kernel.spawn_agent(manifest) {
            Ok(id) => {
                ...
                ui::hint("For persistent agents, use `rustyhand start` first");
            }
            Err(e) => {
...
fn cmd_status(config: Option<PathBuf>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        ui::blank();
        ui::hint("Run `rustyhand start` to launch the daemon");
...
fn cmd_doctor(json: bool, repair: bool) {
    ...
    if !json {
        ui::step("RustyHand Doctor");
        println!();
    }

    let rusty_hand_dir = rusty_hand_home();

    // --- Check 1: RustyHand directory ---
    if rusty_hand_dir.exists() {
        if !json {
            ui::check_ok(&format!(
                "RustyHand directory: {}",
                rusty_hand_dir.display()
            ));
        }
        checks.push(serde_json::json!({"check": "rusty_hand_dir", "status": "ok", "path": rusty_hand_dir.display().to_string()}));
    } else if repair {
        if !json {
            ui::check_fail("RustyHand directory not found.");
        }
        let answer = prompt_input("    Create it now? [Y/n] ");
        if answer.is_empty() || answer.starts_with('y') || answer.starts_with('Y') {
            if std::fs::create_dir_all(&rusty_hand_dir).is_ok() {
                restrict_dir_permissions(&rusty_hand_dir);
                for sub in ["data", "agents"] {
                    let _ = std::fs::create_dir_all(rusty_hand_dir.join(sub));
                }
                if !json {
                    ui::check_ok("Created RustyHand directory");
                }
                repaired = true;
            } else {
                if !json {
        ...
        checks.push(serde_json::json!({"check": "rusty_hand_dir", "status": if repaired { "repaired" } else { "fail" }}));
    } else {
        if !json {
            ui::check_fail("RustyHand directory not found. Run `rustyhand init` first.");
        }
        checks.push(serde_json::json!({"check": "rusty_hand_dir", "status": "fail"}));
        all_ok = false;
    }

    // --- Check 2: .env file exists + permissions ---
    let env_path = rusty_hand_dir.join(".env");
    if env_path.exists() {
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
        ...
        ui::check_warn(".env file not found (create with: rustyhand config set-key <provider>)");
    ...
    if config_path.exists() {
        ...
        match toml::from_str::<toml::Value>(&config_content) {
            ...
            Err(e) => {
                if !json {
                    ...
                    ui::hint("Fix with: rustyhand config edit");
                }
        ...
        if answer.is_empty() || answer.starts_with('y') || answer.starts_with('Y') {
            let default_config = r#"# RustyHand Agent OS configuration
# See https://github.com/ginkida/rustyhand for documentation
...
decay_rate = 0.05
"#;
            let _ = std::fs::create_dir_all(&rusty_hand_dir);
    ...
    if let Some(ref base) = daemon_running {
        ...
        if !json {
            ui::check_warn("Daemon not running (start with `rustyhand start`)");
        }
    ...
    let daemon_json_path = rusty_hand_dir.join("daemon.json");
    ...
    let db_path = rusty_hand_dir.join("data").join("rustyhand.db");
    if db_path.exists() {
    ...
    #[cfg(unix)]
    {
        if let Ok(output) = std::process::Command::new("df")
            .args(["-m", &rusty_hand_dir.display().to_string()])
            .output()
        {
    ...
    // --- Check 12: Config deserialization into KernelConfig ---
    {
        ...
        if config_path.exists() {
            ...
            let config_content = std::fs::read_to_string(&config_path).unwrap_or_default();
            match toml::from_str::<rusty_hand_types::config::KernelConfig>(&config_content) {
                Ok(cfg) => {
                    if !json {
                    ...
                    if !cfg.include.is_empty() {
                        ...
                        for inc in &cfg.include {
                            let inc_path = rusty_hand_dir.join(inc);
                            if inc_path.exists() {
                                if !json {
                    ...
                    if !cfg.mcp_servers.is_empty() {
                        ...
                        for server in &cfg.mcp_servers {
                            ...
                            match &server.transport {
                                rusty_hand_types::config::McpTransportEntry::Stdio {
                                    command,
                                    ..
                                } => {
                                    if command.is_empty() {
    ...
    // --- Check 13: Skill registry health ---
    {
        ...
        let skills_dir = rusty_hand_dir.join("skills");
        let mut skill_reg = rusty_hand_skills::registry::SkillRegistry::new(skills_dir.clone());
        ...
        for skill in &skills {
            if let Some(ref prompt) = skill.manifest.prompt_context {
                let warnings =
                    rusty_hand_skills::verify::SkillVerifier::scan_prompt_content(prompt);
    ...
    // --- Check 14: Extension registry health ---
    {
        ...
        let mut ext_registry =
            rusty_hand_extensions::registry::IntegrationRegistry::new(&rusty_hand_dir);
    ...
    if json {
        ...
        println!();
        if all_ok {
            ui::success("All checks passed! RustyHand is ready.");
            ui::hint("Start the daemon: rustyhand start");
        } else if repaired {
            ui::success("Repairs applied. Re-run `rustyhand doctor` to verify.");
        } else {
            ui::error("Some checks failed.");
            if !repair {
                ui::hint("Run `rustyhand doctor --repair` to attempt auto-fix");
            }
        }
    }
}
        ...
        match start_daemon_background() {
            ...
            Err(e) => {
                ui::error_with_fix(
                    ...
                    "Start it manually: rustyhand start",
                );
                std::process::exit(1);
            }
        }
...
fn cmd_completion(shell: clap_complete::Shell) {
    ...
    clap_complete::generate(shell, &mut cmd, "rustyhand", &mut std::io::stdout());
}
...
fn require_daemon(command: &str) -> String {
    find_daemon().unwrap_or_else(|| {
        ui::error_with_fix(
            &format!("`rustyhand {command}` requires a running daemon"),
            "Start the daemon: rustyhand start",
        );
        ui::hint("Or try `rustyhand chat` which works without a daemon");
        std::process::exit(1);
    })
}

fn boot_kernel(config: Option<PathBuf>) -> RustyHandKernel {
    match RustyHandKernel::boot(config.as_deref()) {
        Ok(k) => k,
        Err(e) => {
            boot_kernel_error(&e);
            std::process::exit(1);
...
fn cmd_skill_install(source: &str) {
    ...
    if source_path.exists() && source_path.is_dir() {
        ...
        if !manifest_path.exists() {
            ...
            if rusty_hand_skills::openclaw_compat::detect_openclaw_skill(&source_path) {
                ...
                match rusty_hand_skills::openclaw_compat::convert_openclaw_skill(&source_path) {
                    Ok(manifest) => {
                        ...
                        if let Err(e) =
                            rusty_hand_skills::openclaw_compat::write_rusty_hand_manifest(
                                &dest, &manifest,
                            )
                        {
        ...
        let manifest: rusty_hand_skills::SkillManifest =
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        let client = rusty_hand_skills::marketplace::MarketplaceClient::new(
            rusty_hand_skills::marketplace::MarketplaceConfig::default(),
        );
...
fn cmd_skill_create() {
    ...
    let entry_file = if runtime == "python" {
        "src/main.py"
    } else {
        "src/index.js"
    };
...
"#
    ...
    println!("  2. Test locally: rustyhand skill test");
    println!(
        "  3. Install: rustyhand skill install {}",
        skill_dir.display()
    );
}
...
fn cmd_channel_list() {
    ...
    if !config_path.exists() {
        println!("No configuration found. Run `rustyhand init` first.");
        return;
    }
...
fn maybe_write_channel_config(channel: &str, config_block: &str) {
    ...
    if !config_path.exists() {
        ui::hint("No config.toml found. Run `rustyhand init` first.");
        return;
    }
...
/// Spawn `rustyhand start` as a detached background process.
///
...
fn cmd_config_show() {
    ...
    if !config_path.exists() {
        ...
        println!("Run `rustyhand init` to create one.");
        return;
    }
...
fn cmd_config_get(key: &str) {
    ...
    if !config_path.exists() {
        ui::error_with_fix("No config file found", "Run `rustyhand init` first");
        std::process::exit(1);
    }
    ...
    let table: toml::Value = toml::from_str(&content).unwrap_or_else(|e| {
        ui::error_with_fix(
            ...
            "Fix your config.toml syntax, or run `rustyhand config edit`",
        );
        std::process::exit(1);
    });
...
fn cmd_config_set_key(provider: &str) {
    ...
    match dotenv::save_env_key(&env_var, &key) {
        Ok(()) => {
            ui::success(&format!("Saved {env_var} to ~/.rustyhand/.env"));
            // Test the key
            print!("  Testing key... ");
...
fn cmd_config_delete_key(provider: &str) {
    ...
    match dotenv::remove_env_key(&env_var) {
        Ok(()) => ui::success(&format!("Removed {env_var} from ~/.rustyhand/.env")),
        Err(e) => {
...
fn cmd_config_test_key(provider: &str) {
    ...
    if std::env::var(&env_var).is_err() {
        ...
        ui::hint(&format!("Set it: rustyhand config set-key {provider}"));
        std::process::exit(1);
    }
    ...
    if test_api_key(provider, &env_var) {
        ...
        ui::hint(&format!("Update key: rustyhand config set-key {provider}"));
        std::process::exit(1);
    }
}
...
pub(crate) fn rusty_hand_home() -> PathBuf {
...
// ---------------------------------------------------------------------------
// Integration commands (rustyhand add/remove/integrations)
// ---------------------------------------------------------------------------

fn cmd_integration_add(name: &str, key: Option<&str>) {
    let home = rusty_hand_home();
    let mut registry = rusty_hand_extensions::registry::IntegrationRegistry::new(&home);
    registry.load_bundled();
    ...
    let vault = if vault_path.exists() {
        let mut v = rusty_hand_extensions::vault::CredentialVault::new(vault_path);
        if v.unlock().is_ok() {
            Some(v)
        } else {
            None
    ...
    let mut resolver =
        rusty_hand_extensions::credentials::CredentialResolver::new(vault, Some(&dotenv_path))
    ...
    match rusty_hand_extensions::installer::install_integration(
        &mut registry,
        &mut resolver,
        name,
        &provided_keys,
        ...
        Ok(result) => {
            match &result.status {
                rusty_hand_extensions::IntegrationStatus::Ready => {
                    ui::success(&result.message);
                }
                rusty_hand_extensions::IntegrationStatus::Setup => {
                    println!("{}", result.message.yellow());
                    println!("\nTo add credentials:");
                    for env in &template.required_env {
                        if env.is_secret {
                            println!("  rustyhand vault set {}  # {}", env.name, env.help);
...
fn cmd_integration_remove(name: &str) {
    ...
    match rusty_hand_extensions::installer::remove_integration(&mut registry, name) {
        Ok(msg) => {
            ui::success(&msg);
            // Hot-reload daemon
            if let Some(base_url) = find_daemon() {
...
fn cmd_integrations_list(query: Option<&str>) {
    ...
    let dotenv_path = home.join(".env");
    let resolver =
        rusty_hand_extensions::credentials::CredentialResolver::new(None, Some(&dotenv_path));

    let entries = if let Some(q) = query {
        rusty_hand_extensions::installer::search_integrations(&registry, q)
    } else {
        rusty_hand_extensions::installer::list_integrations(&registry, &resolver)
    };
    ...
    let mut by_category: std::collections::BTreeMap<
        String,
        Vec<&rusty_hand_extensions::installer::IntegrationListEntry>,
    ...
    for (category, items) in &by_category {
        ...
        for item in items {
            let status_badge = match &item.status {
                rusty_hand_extensions::IntegrationStatus::Ready => "[Ready]".green().to_string(),
                rusty_hand_extensions::IntegrationStatus::Setup => "[Setup]".yellow().to_string(),
                rusty_hand_extensions::IntegrationStatus::Available => {
                    "[Available]".dimmed().to_string()
                }
                rusty_hand_extensions::IntegrationStatus::Error(msg) => {
                    format!("[Error: {msg}]").red().to_string()
                }
                rusty_hand_extensions::IntegrationStatus::Disabled => {
    ...
    println!("  Use `rustyhand add <name>` to install an integration.");
}

// ---------------------------------------------------------------------------
// Vault commands (rustyhand vault init/set/list/remove)
// ---------------------------------------------------------------------------

fn cmd_vault_init() {
    let home = rusty_hand_home();
    let vault_path = home.join("vault.enc");
    let mut vault = rusty_hand_extensions::vault::CredentialVault::new(vault_path);
...
fn cmd_vault_set(key: &str) {
    ...
    if !vault.exists() {
        ui::error("Vault not initialized. Run: rustyhand vault init");
        std::process::exit(1);
    }
...
fn cmd_vault_list() {
    ...
    if !vault.exists() {
        println!("Vault not initialized. Run: rustyhand vault init");
        return;
    }
...
// ---------------------------------------------------------------------------
// Scaffold commands (rustyhand new skill/integration)
// ---------------------------------------------------------------------------
...
fn cmd_scaffold(kind: ScaffoldKind) {
    ...
    let result = match kind {
        ScaffoldKind::Skill => {
            rusty_hand_extensions::installer::scaffold_skill(&cwd.join("my-skill"))
        }
        ScaffoldKind::Integration => {
            rusty_hand_extensions::installer::scaffold_integration(&cwd.join("my-integration"))
        }
    };
    match result {
...
fn cmd_health(json: bool) {
    match find_daemon() {
        ...
        None => {
            ...
            ui::hint("Start it with: rustyhand start");
            std::process::exit(1);
        }
    }
}
...
fn cmd_security_status(json: bool) {
    ...
    ui::kv("Wire protocol", "RHP HMAC-SHA256 mutual auth");
...
fn cmd_security_hash_key(key: Option<String>) {
    let key = match key {
        Some(key) => key,
        None => {
            eprint!("API key: ");
            ...
            if io::stdin().read_line(&mut buf).is_err() {
                ui::error("Failed to read API key from stdin.");
                std::process::exit(1);
            }
            buf.trim().to_string()
        }
    ...
    let hash = Argon2::default()
        ...
        .unwrap_or_else(|err| {
            ui::error(&format!("Failed to hash API key: {err}"));
            std::process::exit(1);
        });
...
fn infer_backup_format(
    path: &std::path::Path,
    format: Option<BackupFormatArg>,
) -> Option<BackupFormatArg> {
    format.or_else(|| {
        match path
            .extension()
            ...
            .as_deref()
        {
            Some("json") => Some(BackupFormatArg::Json),
            Some("msgpack") | Some("messagepack") | Some("mpk") => {
                Some(BackupFormatArg::Messagepack)
            }
            _ => None,
        }
    })
...
fn backup_format_name(format: BackupFormatArg) -> &'static str {
    match format {
        BackupFormatArg::Json => "json",
        BackupFormatArg::Messagepack => "messagepack",
    }
}

fn cmd_memory_export(path: &std::path::Path, format: Option<BackupFormatArg>) {
    let format = infer_backup_format(path, format).unwrap_or(BackupFormatArg::Json);
    let base = require_daemon("memory export");
    let client = daemon_client();
    let url = format!(
        "{base}/api/memory/export?format={}",
        backup_format_name(format)
    );
    ...
    match client.get(url).send() {
        Ok(resp) if resp.status().is_success() => {
            let bytes = match resp.bytes() {
                Ok(bytes) => bytes,
                Err(err) => {
                    ui::error(&format!("Failed to read backup: {err}"));
                    std::process::exit(1);
                }
            };
            ...
            ui::success(&format!("Backup exported to {}.", path.display()));
        }
        Ok(resp) => {
            let body = resp
                .json::<serde_json::Value>()
                .unwrap_or_else(|_| serde_json::json!({ "error": "Backup export failed" }));
            ui::error(body["error"].as_str().unwrap_or("Backup export failed"));
            std::process::exit(1);
        }
        Err(err) => {
            ui::error(&format!("Backup export failed: {err}"));
            std::process::exit(1);
        }
    }
}

fn cmd_memory_import(path: &std::path::Path, format: Option<BackupFormatArg>) {
    let format = infer_backup_format(path, format).unwrap_or_else(|| {
        ui::error("Could not infer backup format. Pass --format json|messagepack.");
        std::process::exit(1);
    });
    ...
    let base = require_daemon("memory import");
    let client = daemon_client();
    let url = format!(
        "{base}/api/memory/import?format={}",
        backup_format_name(format)
    );
    let resp = client
        .post(url)
        .header(
            ...
            match format {
                BackupFormatArg::Json => "application/json",
                BackupFormatArg::Messagepack => "application/x-msgpack",
            },
        )
        .body(data)
        .send();
    ...
    if body.get("error").is_some() {
        ui::error(body["error"].as_str().unwrap_or("Backup import failed"));
        std::process::exit(1);
    }

    ui::success(&format!(
        "Backup imported: {} memories, {} entities, {} relations.",
        body["memories_imported"].as_u64().unwrap_or(0),
        body["entities_imported"].as_u64().unwrap_or(0),
        body["relations_imported"].as_u64().unwrap_or(0),
    ));
}
...
fn cmd_devices_pair() {
    ...
    if let Some(qr) = body["qr_data"].as_str() {
        ...
        println!("  Scan this QR code with the RustyHand mobile app:");
        ui::blank();
        println!("  {qr}");
        ui::blank();
...
fn cmd_system_info(json: bool) {
    if let Some(base) = find_daemon() {
        ...
        ui::section("RustyHand System Info");
        ui::blank();
        ...
        ui::kv("Version", env!("CARGO_PKG_VERSION"));
        ui::kv_warn("Daemon", "NOT RUNNING");
        ui::hint("Start with: rustyhand start");
    }
}
...
fn cmd_system_version(json: bool) {
    ...
    println!("rustyhand {}", env!("CARGO_PKG_VERSION"));
}
...
fn cmd_reset(confirm: bool) {
    ...
    match std::fs::remove_dir_all(&rusty_hand_dir) {
        Ok(()) => ui::success(&format!("Removed {}", rusty_hand_dir.display())),
        Err(e) => {
            ui::error(&format!(
                "Failed to remove {}: {e}",
                rusty_hand_dir.display()
...
mod tests {
    ...
    fn test_doctor_skill_registry_loads_bundled() {
        let skills_dir = std::env::temp_dir().join("rusty-hand-doctor-test-skills");
        let mut skill_reg = rusty_hand_skills::registry::SkillRegistry::new(skills_dir);
    ...
    fn test_doctor_extension_registry_loads_bundled() {
        let tmp = std::env::temp_dir().join("rusty-hand-doctor-test-ext");
        let _ = std::fs::create_dir_all(&tmp);
        let mut ext_reg = rusty_hand_extensions::registry::IntegrationRegistry::new(&tmp);
    ...
    fn test_doctor_config_deser_default() {
        ...
        let parsed: rusty_hand_types::config::KernelConfig = toml::from_str(&toml_str).unwrap();
...
api_key_env = "GROQ_API_KEY"
"#;
        let config: rusty_hand_types::config::KernelConfig = toml::from_str(config_toml).unwrap();
...
"#;
        ...
        assert_eq!(
            ...
            rusty_hand_types::config::ExecSecurityMode::Allowlist
        );
...
"#;
    ...
    fn test_doctor_skill_injection_scan_clean() {
        ...
        let warnings = rusty_hand_skills::verify::SkillVerifier::scan_prompt_content(clean_content);
    ...
    fn test_doctor_hook_event_variants() {
        ...
        use rusty_hand_types::agent::HookEvent;
        let events = [
