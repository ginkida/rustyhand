//! Minimal `.env` file loader/saver for the RustyHand home directory.
//!
...
/// Return the path to `RUSTY_HAND_HOME/.env`.
pub fn env_file_path() -> Option<PathBuf> {
    Some(rusty_hand_types::config::rusty_hand_home_dir().join(".env"))
}

/// Load `RUSTY_HAND_HOME/.env` and `RUSTY_HAND_HOME/secrets.env` into `std::env`.
///
...
/// Return the path to `RUSTY_HAND_HOME/secrets.env`.
pub fn secrets_env_path() -> Option<PathBuf> {
    Some(rusty_hand_types::config::rusty_hand_home_dir().join("secrets.env"))
}
...
/// Remove a key from `~/.rustyhand/.env`.
///
...
/// List key names (without values) from `~/.rustyhand/.env`.
#[allow(dead_code)]
...
fn write_env_file(path: &PathBuf, entries: &BTreeMap<String, String>) -> Result<(), String> {
    let mut content =
        String::from("# RustyHand environment — managed by `rustyhand config set-key`\n");
