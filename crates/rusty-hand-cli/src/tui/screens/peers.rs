//! Peers screen: RHP peer network status with auto-refresh.
    ...
    f.render_widget(
        Paragraph::new(vec![
            Line::from(vec![Span::styled(
                format!("  RHP Peer Network  ({} peers)", state.peers.len()),
                Style::default()
                    .fg(theme::CYAN)
    ...
    if state.loading && state.peers.is_empty() {
        ...
        f.render_widget(
            Paragraph::new(Span::styled(
                "  No peers connected. Configure [network] in config.toml to enable RHP.",
                theme::dim_style(),
            )),
            chunks[1],
        );
