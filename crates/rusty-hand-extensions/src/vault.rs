...
//!
//! Stores secrets in `~/.rustyhand/vault.enc`, with the master key sourced from
//! the OS keyring (Windows Credential Manager / macOS Keychain / Linux Secret Service)
//! or the `RUSTY_HAND_VAULT_KEY` env var for headless/CI environments.
...
impl CredentialVault {
    ...
    pub fn unlock(&mut self) -> ExtensionResult<()> {
        ...
        if !self.path.exists() {
            return Err(ExtensionError::Vault(
                "Vault not initialized. Run `rustyhand vault init`.".to_string(),
            ));
        }
...
fn machine_fingerprint() -> Vec<u8> {
    ...
    hasher.update(b"rusty-hand-vault-v1");
