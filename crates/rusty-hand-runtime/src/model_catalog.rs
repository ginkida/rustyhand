...
impl ModelCatalog {
    ...
    pub fn detect_auth(&mut self) {
        // Check env var is set AND non-empty (empty string = not configured)
        ...
        for provider in &mut self.providers {
            if !provider.key_required {
                ...
                if provider.id == "gemini" && env_key_present("GOOGLE_API_KEY") {
                    provider.auth_status = AuthStatus::Configured;
                } else {
...
fn builtin_models() -> Vec<ModelCatalogEntry> {
    vec![
        ...
        ModelCatalogEntry {
            ...
            supports_vision: false,
            supports_streaming: true,
            aliases: vec!["minimax".into(), "m2.7".into()],
        },
        ModelCatalogEntry {
            id: "MiniMax-M2.7-highspeed".into(),
            display_name: "MiniMax M2.7 Highspeed".into(),
            provider: "minimax".into(),
            tier: ModelTier::Frontier,
            context_window: 204_800,
            max_output_tokens: 131_072,
