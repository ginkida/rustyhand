...
//! - `build_agent_card` — expose RustyHand agents via A2A
...
pub struct AgentCard {
    ...
    /// Skills this agent can perform (A2A skill descriptors, not RustyHand skills).
...
/// A2A skill descriptor (not an RustyHand skill — describes a capability).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub async fn discover_external_agents(
    agents: &[rusty_hand_types::config::ExternalAgent],
...
// ---------------------------------------------------------------------------
// A2A Server — expose RustyHand agents via A2A
// ---------------------------------------------------------------------------

/// Build an A2A Agent Card from an RustyHand agent manifest.
...
impl A2aClient {
    ...
    pub async fn discover(&self, url: &str) -> Result<AgentCard, String> {
        ...
        let response = self
            .client
            .get(&agent_json_url)
            .header("User-Agent", "RustyHand/0.1 A2A")
            .send()
            .await
...
mod tests {
    ...
    fn test_a2a_config_serde() {
        use rusty_hand_types::config::{A2aConfig, ExternalAgent};
