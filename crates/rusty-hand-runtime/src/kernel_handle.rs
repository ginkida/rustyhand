...
//!
//! This trait allows `rusty-hand-runtime` to call back into the kernel for
...
pub trait KernelHandle: Send + Sync {
    ...
    /// Update an existing cron job (preserves ID and history).
    async fn cron_update(
        &self,
        job_id: &str,
        job_json: serde_json::Value,
    ) -> Result<String, String> {
        let _ = (job_id, job_json);
        Err("Cron scheduler not available".to_string())
    }
    ...
    /// List all cron jobs across agents.
    ...
    /// Ingest document chunks into semantic memory with embeddings.
    ...
    async fn doc_ingest(
        ...
        let _ = (agent_id, source, chunks);
        Err("Document ingest not available".to_string())
    }
    ...
    async fn doc_search(
        ...
        Err("Document search not available".to_string())
    }
    ...
    /// Get usage metrics for a specific agent. Returns JSON with token counts, cost, etc.
