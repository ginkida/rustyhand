...
use rusty_hand_types::config::{ExecPolicy, ExecSecurityMode};
...
async fn kill_tree_unix(pid: u32, grace_ms: u64) -> Result<bool, String> {
    fn is_alive(pid: i32) -> bool {
        // SAFETY: libc::kill is called with a PID and signal 0 for existence check.
    ...
    fn send_signal(pid: i32, sig: libc::c_int) {
        // SAFETY: libc::kill is called with validated integer PID/signal.
        let _ = unsafe { libc::kill(pid, sig) };
    }

    let pid_i32 = i32::try_from(pid).map_err(|_| format!("Invalid PID value: {pid}"))?;

    // If target already does not exist, report as not killed.
    if !is_alive(pid_i32) {
        return Ok(false);
    }

    // Graceful termination: first try process group, then direct PID.
    send_signal(-pid_i32, libc::SIGTERM);
    send_signal(pid_i32, libc::SIGTERM);
    ...
    if is_alive(pid_i32) {
        ...
        send_signal(-pid_i32, libc::SIGKILL);
        send_signal(pid_i32, libc::SIGKILL);
        Ok(true)
    } else {
        Ok(true)
...
/// This is the preferred way to clean up subprocesses spawned by RustyHand.
...
pub async fn wait_or_kill_with_idle(
    ...
    grace_ms: u64,
) -> Result<(rusty_hand_types::config::TerminationReason, String), String> {
    ...
    loop {
        ...
        if tokio::time::Instant::now() >= deadline {
            ...
            kill_child_tree(child, grace_ms).await?;
            return Ok((
                rusty_hand_types::config::TerminationReason::AbsoluteTimeout,
                output,
            ));
        }
        ...
        if let Some(idle_dl) = idle_deadline {
            if tokio::time::Instant::now() >= idle_dl {
                ...
                kill_child_tree(child, grace_ms).await?;
                return Ok((
                    rusty_hand_types::config::TerminationReason::NoOutputTimeout,
                    output,
                ));
            }
        }
                    ...
                    Ok(0) => {
                        ...
                        if stderr.is_none() {
                            ...
                            match tokio::time::timeout(
                                ...
                                Ok(Ok(status)) => {
                                    return Ok((
                                        rusty_hand_types::config::TerminationReason::Exited(status.code().unwrap_or(-1)),
                                        output,
                                    ));
                                }
                                ...
                                Err(_) => {
                                    kill_child_tree(child, grace_ms).await?;
                                    return Ok((rusty_hand_types::config::TerminationReason::AbsoluteTimeout, output));
                                }
                            }
                        }
                    }
...
mod tests {
    ...
    async fn test_kill_nonexistent_process() {
        // Killing a non-existent PID should not panic and should report not killed.
