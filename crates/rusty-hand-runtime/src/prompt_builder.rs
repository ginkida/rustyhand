...
fn build_identity_section(ctx: &PromptContext) -> String {
    if ctx.base_system_prompt.is_empty() {
        format!(
            "You are {}, an AI agent running inside the RustyHand Agent OS.\n{}",
...
pub fn tool_category(name: &str) -> &'static str {
    match name {
        ...
        "image_analyze" | "media_describe" | "media_transcribe" | "image_generate"
        | "text_to_speech" | "speech_to_text" => "Media",
...
pub fn tool_hint(name: &str) -> &'static str {
    match name {
        ...
        // Media
        "image_analyze" => "analyze an image file",
        "media_describe" => "describe an image with a vision model",
        "media_transcribe" => "transcribe audio to text",
        ...
        "cron_cancel" => "remove a scheduled task",
...
mod tests {
    ...
    fn test_tool_categories() {
        ...
        assert_eq!(tool_category("media_describe"), "Media");
        assert_eq!(tool_category("schedule_create"), "Scheduling");
    ...
    fn test_tool_hints() {
        ...
        assert_eq!(tool_hint("media_transcribe"), "transcribe audio to text");
        assert_eq!(tool_hint("schedule_delete"), "remove a scheduled task");
        assert_eq!(tool_hint("cron_cancel"), "remove a scheduled task");
    ...
    #[test]
    fn test_cap_str_unicode() {
        let result = cap_str("🙂🙂🙂🙂", 3);
        assert_eq!(result, "🙂🙂🙂...");
    }
