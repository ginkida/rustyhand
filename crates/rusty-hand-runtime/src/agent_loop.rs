...
use rusty_hand_memory::session::Session;
...
use rusty_hand_types::memory::{Memory, MemoryFilter, MemorySource};
use rusty_hand_types::message::{
    ContentBlock, Message, MessageContent, Role, StopReason, TokenUsage,
};
use rusty_hand_types::tool::{ToolCall, ToolDefinition};
...
pub struct AgentLoopResult {
    ...
    pub directives: rusty_hand_types::message::ReplyDirectives,
}

/// Run the agent execution loop for a single user message.
///
/// This is the core of RustyHand: it loads session context, recalls memories,
...
pub async fn run_agent_loop(
    ...
    global_exec_policy: Option<&rusty_hand_types::config::ExecPolicy>,
) -> RustyHandResult<AgentLoopResult> {
    info!(agent = %manifest.name, "Starting agent loop");

    // Extract optional env allowlist from manifest metadata.
    let allowed_env: Vec<String> = manifest
        .metadata
        .get("allowed_env")
    ...
    if let Some(hook_reg) = hooks {
        let ctx = crate::hooks::HookContext {
            ...
            event: rusty_hand_types::agent::HookEvent::BeforePromptBuild,
            ...
            StopReason::EndTurn | StopReason::StopSequence => {
                ...
                if text.trim() == "NO_REPLY" || parsed_directives.silent {
                    ...
                    return Ok(AgentLoopResult {
                        ...
                        cost_usd: None,
                        silent: true,
                        directives: rusty_hand_types::message::ReplyDirectives {
                ...
                if let Some(hook_reg) = hooks {
                    let ctx = crate::hooks::HookContext {
                        agent_name: &manifest.name,
                        agent_id: agent_id_str.as_str(),
                        event: rusty_hand_types::agent::HookEvent::AgentLoopEnd,
                        data: serde_json::json!({
                            "iterations": iteration + 1,
                ...
                for tool_call in &response.tool_calls {
                    ...
                    match &verdict {
                        LoopGuardVerdict::CircuitBreak(msg) => {
                            ...
                            return Err(RustyHandError::Internal(msg.clone()));
                        }
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: rusty_hand_types::agent::HookEvent::BeforeToolCall,
                            data: serde_json::json!({
                    ...
                    // Resolve effective exec policy (per-agent override → global fallback)
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: rusty_hand_types::agent::HookEvent::AfterToolCall,
                            data: serde_json::json!({
                                "tool_name": &tool_call.name,
    ...
    for attempt in 0..=MAX_RETRIES {
        match driver.complete(request.clone()).await {
            ...
            Err(e) => {
                ...
                return Err(RustyHandError::LlmDriver(user_msg));
            }
        }
    }

    Err(RustyHandError::LlmDriver(
            ...
            StopReason::EndTurn | StopReason::StopSequence => {
                ...
                return Ok(AgentLoopResult {
                    ...
                    directives: rusty_hand_types::message::ReplyDirectives {
                        reply_to: parsed_directives_s.reply_to,
                        current_thread: parsed_directives_s.current_thread,
                        silent: parsed_directives_s.silent,
                    },
                });
            }
            StopReason::ToolUse => {
                ...
                for tool_call in &response.tool_calls {
                    ...
                    let preview: String = final_content.chars().take(2000).collect();
                    if stream_tx
...
mod tests {
    ...
    /// Mock driver that returns reply directives in the text payload.
    struct DirectivesDriver {
        text: String,
    }

    #[async_trait]
    impl LlmDriver for DirectivesDriver {
        async fn complete(
            &self,
            _request: CompletionRequest,
        ) -> Result<CompletionResponse, LlmError> {
    ...
    async fn test_empty_response_after_tool_use_returns_fallback() {
        let memory = rusty_hand_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
    ...
    #[tokio::test]
    async fn test_reply_directives_propagated_non_streaming() {
        let memory = rusty_hand_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = rusty_hand_types::agent::AgentId::new();
        let mut session = rusty_hand_memory::session::Session {
            id: rusty_hand_types::agent::SessionId::new(),
        ...
        let manifest = test_manifest();
        let driver: Arc<dyn LlmDriver> = Arc::new(DirectivesDriver {
            text: "[[reply:msg-42]] [[@current]] Hello from directives".to_string(),
        });
        ...
        assert_eq!(result.response, "Hello from directives");
        assert_eq!(result.directives.reply_to.as_deref(), Some("msg-42"));
    ...
    #[tokio::test]
    async fn test_reply_directives_propagated_streaming() {
        let memory = rusty_hand_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = rusty_hand_types::agent::AgentId::new();
        let mut session = rusty_hand_memory::session::Session {
            id: rusty_hand_types::agent::SessionId::new(),
        ...
        let manifest = test_manifest();
        let driver: Arc<dyn LlmDriver> = Arc::new(DirectivesDriver {
            text: "[[reply:msg-stream]] [[@current]] Streaming directives".to_string(),
        });
        let (tx, _rx) = mpsc::channel(64);
        ...
        assert_eq!(result.response, "Streaming directives");
        assert_eq!(result.directives.reply_to.as_deref(), Some("msg-stream"));
        assert!(result.directives.current_thread);
        assert!(!result.directives.silent);
    }
