...
use rusty_hand_skills::registry::SkillRegistry;
use rusty_hand_types::taint::{TaintLabel, TaintSink, TaintedValue};
use rusty_hand_types::tool::{ToolDefinition, ToolResult};
...
fn check_taint_shell_exec(command: &str) -> Option<String> {
    ...
    let suspicious_patterns = [
        ...
        "`curl",
        "eval ",
        // Scripting language injection vectors
        "python -c",
        "python3 -c",
        "perl -e",
        "ruby -e",
...
///
/// URLs targeting hosts in `trusted_hosts` skip this check — the admin has
/// explicitly allowed agents to authenticate against those services.
fn check_taint_net_fetch(url: &str, trusted_hosts: &[String]) -> Option<String> {
    // If the URL targets a trusted host, skip taint check
...
pub async fn execute_tool(
    ...
    exec_policy: Option<&rusty_hand_types::config::ExecPolicy>,
    tts_engine: Option<&crate::tts::TtsEngine>,
    docker_config: Option<&rusty_hand_types::config::DockerSandboxConfig>,
    ...
    // SECURITY: enforce depth-based tool restrictions for subagents.
    // Dangerous admin/scheduling tools are denied to all subagents (depth > 0).
    // Spawn/kill tools are additionally denied at leaf depth to prevent deep chains.
    {
        let depth = current_agent_depth();
        if depth > 0 {
            use crate::tool_policy::filter_tools_by_depth;
            let tool_vec = vec![tool_name.to_string()];
            let filtered = filter_tools_by_depth(&tool_vec, depth, MAX_AGENT_CALL_DEPTH);
            if filtered.is_empty() {
                ...
                return ToolResult {
                    tool_use_id: tool_use_id.to_string(),
                    content: format!(
                        "Permission denied: tool '{tool_name}' is not available at subagent depth {depth}"
                    ),
                    is_error: true,
                };
            }
    ...
    let result = match tool_name {
        ...
        "web_fetch" => {
            ...
            let trusted = web_ctx.map(|c| c.fetch.ssrf_allowlist()).unwrap_or(&[]);
            if let Some(violation) = check_taint_net_fetch(url, trusted) {
                return ToolResult {
                    tool_use_id: tool_use_id.to_string(),
        ...
        "self_history" => tool_self_history(input, kernel, caller_agent_id),
        "self_evaluate" => tool_self_evaluate(input, kernel, caller_agent_id),
        "self_metrics" => tool_self_metrics(kernel, caller_agent_id),
        "self_update" => tool_self_update(input, kernel, caller_agent_id),
        ...
        // Browser automation tools (via agent-browser CLI)
        "browser_navigate" => {
            let url = input["url"].as_str().unwrap_or("");
            let trusted = web_ctx.map(|c| c.fetch.ssrf_allowlist()).unwrap_or(&[]);
            if let Some(violation) = check_taint_net_fetch(url, trusted) {
        ...
        other => {
            ...
            // Fallback 2: Skill registry tool providers
                ...
                if let Some(skill) = registry.find_tool_provider(other) {
                    ...
                    match rusty_hand_skills::loader::execute_skill_tool(
                        &skill.manifest,
                        &skill.path,
                        other,
                        input,
...
pub fn builtin_tool_definitions() -> Vec<ToolDefinition> {
    vec![
        ...
        // --- Document RAG tools ---
        ToolDefinition {
            name: "doc_ingest".to_string(),
            description: "Ingest a document for semantic search. Text is split into overlapping chunks, embedded, and stored. Use for building a searchable knowledge base from files, web pages, or any text.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "content": { "type": "string", "description": "The document text to ingest (max 512KB)" },
                    "source": { "type": "string", "description": "Source name for reference (e.g. filename, URL)" },
                    "chunk_size": { "type": "integer", "description": "Chunk size in characters (default 1600, ~400 tokens)" },
                    "overlap": { "type": "integer", "description": "Overlap between chunks in characters (default 200)" }
                },
                "required": ["content"]
            }),
        },
        ToolDefinition {
            ...
            description: "Search ingested documents using semantic similarity. Returns the most relevant chunks with similarity scores. Use after doc_ingest to find information in your knowledge base.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
        ...
        // --- Self-improvement tools ---
        ToolDefinition {
            name: "self_history".to_string(),
            description: "Read your own past conversation messages. Use to review what worked, what failed, and learn from patterns.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "last_n": {
        ...
        ToolDefinition {
            ...
            description: "Rate your own performance on the current interaction. Stores a running average. Use after completing a task to track improvement over time.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "rating": {
                    ...
                    "note": {
                        "type": "string",
                        "description": "Brief note on what went well or poorly"
                    }
                },
                "required": ["rating"]
            }),
        ...
        ToolDefinition {
            name: "self_metrics".to_string(),
            description: "Get your own usage metrics: token counts, cost, tool calls, uptime. Use to evaluate your performance and decide whether to optimize.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {},
            }),
        ...
        ToolDefinition {
            ...
            description: "Update your own system prompt. Use to refine your behavior based on experience. Changes take effect on the next message. Store the reason in memory for audit.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "system_prompt": {
                        "type": "string",
                        "description": "The new system prompt to adopt. Must be non-empty and under 32KB."
                    }
                },
        ...
        // --- Browser automation tools (via agent-browser) ---
        ToolDefinition {
            name: "browser_navigate".to_string(),
            description: "Navigate a browser to a URL. Returns an accessibility snapshot with element refs (@e1, @e2, etc.) for interaction. Opens a persistent browser session.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
        ...
        ToolDefinition {
            ...
            description: "Get the accessibility tree of the current page with element refs (@e1, @e2, etc.). Best way to understand page structure and find elements to interact with. Use refs from the snapshot in browser_click and browser_type.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "interactive": { "type": "boolean", "description": "Only show interactive elements like buttons, links, inputs (default: true)" },
        ...
        ToolDefinition {
            ...
            description: "Click an element. Use @ref from snapshot (e.g., '@e2') for reliable selection, or CSS selector, or visible text.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "selector": { "type": "string", "description": "Element ref from snapshot (e.g., '@e2'), CSS selector (e.g., '#submit'), or visible text" }
                },
                "required": ["selector"]
            }),
        },
        ToolDefinition {
            ...
            description: "Clear and fill text into an input field. Use @ref from snapshot (e.g., '@e3') or CSS selector.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "selector": { "type": "string", "description": "Element ref (e.g., '@e3') or CSS selector for the input field" },
        ...
        ToolDefinition {
            ...
            description: "Take a screenshot of the current browser page. Returns a PNG image URL.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {}
            }),
        ...
        ToolDefinition {
            ...
            description: "Read the current page as an accessibility snapshot with element refs. Use after clicking or navigating to see the updated page state.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {}
            }),
        ...
        ToolDefinition {
            ...
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "url": { "type": "string", "description": "Base URL of the remote RustyHand/A2A-compatible agent (e.g., 'https://agent.example.com')" }
                },
                "required": ["url"]
            }),
        },
...
/// Maximum key length for memory_store (256 chars).
const MEMORY_KEY_MAX_LEN: usize = 256;
/// Maximum serialized value size for memory_store (64 KB).
const MEMORY_VALUE_MAX_BYTES: usize = 64 * 1024;
...
fn tool_memory_store(
    ...
    // SECURITY: enforce size limits to prevent disk exhaustion
    if key.len() > MEMORY_KEY_MAX_LEN {
        return Err(format!(
            "Key too long ({} chars, max {MEMORY_KEY_MAX_LEN})",
            key.len()
        ));
    }
    ...
    if serialized_size > MEMORY_VALUE_MAX_BYTES {
        return Err(format!(
            "Value too large ({serialized_size} bytes, max {MEMORY_VALUE_MAX_BYTES})"
        ));
    }
...
/// Maximum document size for ingestion (512 KB).
const DOC_INGEST_MAX_BYTES: usize = 512 * 1024;
/// Default chunk size in characters (~400 tokens).
const DOC_CHUNK_SIZE: usize = 1600;
...
async fn tool_doc_ingest(
    input: &serde_json::Value,
    kernel: Option<&Arc<dyn KernelHandle>>,
    caller_agent_id: Option<&str>,
) -> Result<String, String> {
    let kh = require_kernel(kernel)?;
    let agent_id = caller_agent_id.ok_or("doc_ingest requires a caller agent ID")?;
    let content = input["content"]
        .as_str()
        .ok_or("Missing 'content' parameter")?;
    let source = input["source"].as_str().unwrap_or("unnamed_document");

    if content.len() > DOC_INGEST_MAX_BYTES {
        return Err(format!(
            "Document too large ({} bytes, max {DOC_INGEST_MAX_BYTES})",
            content.len()
        ));
    }
    ...
    let stored = kh.doc_ingest(agent_id, source, chunks.clone()).await?;
    Ok(format!(
        "Ingested '{source}': {stored} chunks stored ({} chars total).",
        content.len()
    ))
}

async fn tool_doc_search(
    input: &serde_json::Value,
    kernel: Option<&Arc<dyn KernelHandle>>,
    caller_agent_id: Option<&str>,
) -> Result<String, String> {
    ...
    let results = kh.doc_search(agent_id, query, limit).await?;
    if results.is_empty() {
...
fn tool_self_history(
    input: &serde_json::Value,
    kernel: Option<&Arc<dyn KernelHandle>>,
    caller_agent_id: Option<&str>,
) -> Result<String, String> {
    let kh = require_kernel(kernel)?;
    let agent_id = caller_agent_id.ok_or("self_history requires a caller agent ID")?;
    let last_n = input["last_n"].as_u64().unwrap_or(20) as usize;
    let last_n = last_n.min(100); // cap at 100 messages
    let history = kh.agent_history(agent_id, last_n)?;
    serde_json::to_string_pretty(&history).map_err(|e| format!("Serialize failed: {e}"))
}

fn tool_self_evaluate(
    input: &serde_json::Value,
    kernel: Option<&Arc<dyn KernelHandle>>,
    caller_agent_id: Option<&str>,
) -> Result<String, String> {
    let kh = require_kernel(kernel)?;
    let agent_id = caller_agent_id.ok_or("self_evaluate requires a caller agent ID")?;
    let rating = input["rating"]
        .as_f64()
        .ok_or("Missing 'rating' (0.0-1.0)")?;
    if !(0.0..=1.0).contains(&rating) {
        return Err("rating must be between 0.0 and 1.0".to_string());
    }
    let note = input["note"].as_str().unwrap_or("");

    // Store evaluation in agent-scoped memory with running stats
    let eval_key = format!("_self_eval:{agent_id}");
    let prev: serde_json::Value = kh
        .memory_recall(&eval_key)?
        .unwrap_or(serde_json::json!({"count": 0, "sum": 0.0, "history": []}));

    let count = prev["count"].as_u64().unwrap_or(0) + 1;
    let sum = prev["sum"].as_f64().unwrap_or(0.0) + rating;
    ...
    kh.memory_store(&eval_key, updated)?;

    Ok(format!(
        "Evaluation recorded: rating={rating:.2}, running avg={avg:.2} over {count} evals."
    ))
}

fn tool_self_metrics(
    kernel: Option<&Arc<dyn KernelHandle>>,
    caller_agent_id: Option<&str>,
) -> Result<String, String> {
    let kh = require_kernel(kernel)?;
    let agent_id = caller_agent_id.ok_or("self_metrics requires a caller agent ID")?;
    let metrics = kh.agent_metrics(agent_id)?;
    serde_json::to_string_pretty(&metrics).map_err(|e| format!("Serialize failed: {e}"))
}

/// Maximum system prompt length for self_update (32 KB).
const SELF_UPDATE_MAX_PROMPT_BYTES: usize = 32 * 1024;
...
fn tool_self_update(
    ...
    let kh = require_kernel(kernel)?;
    let agent_id = caller_agent_id.ok_or("self_update requires a caller agent ID")?;

    let new_prompt = input["system_prompt"]
        .as_str()
    ...
    if new_prompt.len() > SELF_UPDATE_MAX_PROMPT_BYTES {
        return Err(format!(
            "system_prompt too large ({} bytes, max {SELF_UPDATE_MAX_PROMPT_BYTES})",
            new_prompt.len()
        ));
    }

    kh.update_agent_system_prompt(agent_id, new_prompt)?;
    Ok("System prompt updated. Changes take effect on the next message.".to_string())
}
...
fn parse_entity_type(s: &str) -> rusty_hand_types::memory::EntityType {
    use rusty_hand_types::memory::EntityType;
...
fn parse_relation_type(s: &str) -> rusty_hand_types::memory::RelationType {
    use rusty_hand_types::memory::RelationType;
    match s.to_lowercase().as_str() {
...
async fn tool_knowledge_add_entity(
    ...
    let entity = rusty_hand_types::memory::Entity {
...
async fn tool_knowledge_query(
    ...
    let pattern = rusty_hand_types::memory::GraphPattern {
        source,
        relation,
        target,
        max_depth,
...
fn resolve_schedule_target_agent_id(
    kh: &Arc<dyn KernelHandle>,
    agent_hint: Option<&str>,
    caller_agent_id: Option<&str>,
) -> Result<String, String> {
    let hint = agent_hint.map(str::trim).filter(|value| !value.is_empty());
    if let Some(hint) = hint {
        ...
        let matches = kh.find_agents(hint);
        return match matches.len() {
            1 => Ok(matches[0].id.clone()),
            0 => Err(format!("Agent '{hint}' not found.")),
            n => Err(format!(
                "Agent reference '{hint}' is ambiguous ({n} matches). Use the agent ID."
            )),
        };
    }
    ...
    caller_agent_id
        .map(str::to_string)
        .ok_or("Missing 'agent' parameter outside agent context".to_string())
}

fn describe_schedule_json(schedule: &serde_json::Value) -> String {
...
fn extract_schedule_job_id(result: &str) -> Option<String> {
...
async fn tool_schedule_create(
    ...
    let payload = serde_json::json!({
        "name": sanitize_schedule_name(description),
        "schedule": {
            "kind": "cron",
    ...
    let result = kh.cron_create(&agent_id, payload).await?;
    let schedule_id = extract_schedule_job_id(&result).unwrap_or_else(|| "unknown".to_string());
    Ok(format!(
...
async fn tool_schedule_list(kernel: Option<&Arc<dyn KernelHandle>>) -> Result<String, String> {
    let kh = require_kernel(kernel)?;
    let jobs = kh.cron_list_all().await?;
    ...
    for job in &jobs {
        ...
        let agent_id = job["agent_id"].as_str().unwrap_or("");
        let agent_name = agents
            .iter()
            .find(|agent| agent.id == agent_id)
            .map(|agent| agent.name.as_str())
            .unwrap_or(agent_id);
        let next_run = job["next_run"].as_str().unwrap_or("pending");
        output.push_str(&format!(
            "  [{status}] {} — {}\n    Schedule: {} | Agent: {} | Next: {}\n\n",
            job["id"].as_str().unwrap_or("?"),
            job["name"].as_str().unwrap_or("?"),
            describe_schedule_json(&job["schedule"]),
            agent_name,
            next_run,
        ));
    }
...
async fn tool_media_describe(
    ...
    let attachment = rusty_hand_types::media::MediaAttachment {
        media_type: rusty_hand_types::media::MediaType::Image,
        mime_type: mime.to_string(),
        source: rusty_hand_types::media::MediaSource::Base64 {
...
async fn tool_image_generate(
    ...
    let model = match model_str {
        "dall-e-3" | "dalle3" | "dalle-3" => rusty_hand_types::media::ImageGenModel::DallE3,
        "dall-e-2" | "dalle2" | "dalle-2" => rusty_hand_types::media::ImageGenModel::DallE2,
        "gpt-image-1" | "gpt_image_1" => rusty_hand_types::media::ImageGenModel::GptImage1,
        _ => {
            return Err(format!(
    ...
    let request = rusty_hand_types::media::ImageGenRequest {
...
async fn tool_process_start(
    ...
    // SECURITY: Enforce exec policy on process_start (same rules as shell_exec)
    if let Some(policy) = exec_policy {
        use rusty_hand_types::config::ExecSecurityMode;
        match policy.mode {
            ExecSecurityMode::Deny => {
                return Err("process_start blocked: exec_policy mode is 'deny'".to_string());
            }
            ExecSecurityMode::Allowlist => {
                let cmd_base = command.rsplit('/').next().unwrap_or(command);
                if !policy.allowed_commands.iter().any(|c| c == cmd_base)
                    && !policy.safe_bins.iter().any(|c| c == cmd_base)
                {
                    return Err(format!(
                        "process_start blocked: '{cmd_base}' is not in exec_policy allowlist"
                    ));
                }
            }
            ExecSecurityMode::Full => {} // allow all
        }
    }

    let env_vars = allowed_env_vars.unwrap_or(&[]);
    let proc_id = pm.start(agent_id, command, &args, env_vars).await?;
    Ok(serde_json::json!({
        "process_id": proc_id,
        "status": "started"
    })
...
mod tests {
    ...
    async fn test_canvas_present_tool() {
        ...
        let tmp = std::env::temp_dir().join("rusty_hand_canvas_test");
    ...
    fn test_chunk_text_splits_with_overlap() {
        ...
        for pair in chunks.windows(2) {
            ...
            assert!(pair[0].len() <= 700, "Chunk too large: {}", pair[0].len());
        }
    }

    #[test]
    fn test_chunk_text_respects_paragraph_boundary() {
        let text = format!("{}\n\n{}", "Word ".repeat(200), "More ".repeat(200));
        let chunks = chunk_text(&text, 600, 100);
