...
fn find_last_newline_in_char_window(
    text: &str,
    start_char: usize,
    end_char: usize,
) -> Option<usize> {
...
pub fn truncate_tool_result_dynamic(content: &str, budget: &ContextBudget) -> String {
    ...
    let break_point = find_last_newline_in_char_window(content, search_start, cap)
        .unwrap_or_else(|| byte_index_at_char(content, cap.saturating_sub(100)));
    let shown_chars = char_count(&content[..break_point]);
...
pub fn apply_context_guard(
        ...
        if loc.char_len > single_max {
            if let MessageContent::Blocks(blocks) = &mut messages[loc.msg_idx].content {
                if let ContentBlock::ToolResult { content, .. } = &mut blocks[loc.block_idx] {
                    let old_len = char_count(content);
    ...
    for loc in &locations {
        ...
        if let MessageContent::Blocks(blocks) = &mut messages[loc.msg_idx].content {
            if let ContentBlock::ToolResult { content, .. } = &mut blocks[loc.block_idx] {
                if char_count(content) > compact_target {
                    let old_len = char_count(content);
...
fn truncate_to(content: &str, max_chars: usize) -> String {
    ...
    let break_point = find_last_newline_in_char_window(content, keep.saturating_sub(100), keep)
        .unwrap_or_else(|| byte_index_at_char(content, keep));
    let shown_chars = char_count(&content[..break_point]);
    format!(
...
mod tests {
    ...
    #[test]
    fn test_truncate_tool_result_dynamic_handles_unicode() {
        let budget = ContextBudget::new(20); // cap = 12 chars
        let content = "🙂🙂🙂🙂🙂🙂🙂🙂🙂🙂🙂🙂🙂🙂";
        let result = truncate_tool_result_dynamic(content, &budget);
        let preview = result.split("\n\n[TRUNCATED:").next().unwrap();

        assert!(result.contains("[TRUNCATED:"));
        assert!(preview.chars().count() <= budget.per_result_cap());
        assert!(content.starts_with(preview));
    }

    #[test]
    fn test_truncate_to_handles_unicode() {
        let content = "🙂".repeat(120);
        let result = truncate_to(&content, 90);
        let preview = result.split("\n\n[COMPACTED:").next().unwrap();

        assert!(result.contains("[COMPACTED:"));
        assert!(preview.chars().count() <= 90);
    }
}
