...
#   docker build -f scripts/docker/install-smoke.Dockerfile \
#     --build-arg RUSTY_HAND_SMOKE_FULL=1 .
...
ARG RUSTY_HAND_SMOKE_FULL=0
RUN if [ "$RUSTY_HAND_SMOKE_FULL" = "1" ]; then \
        bash /tmp/install.sh; \
    else \
        # 1. Syntax check
...
RUN if [ "$RUSTY_HAND_SMOKE_FULL" = "1" ] && [ -f "$HOME/.rustyhand/bin/rustyhand" ]; then \
        $HOME/.rustyhand/bin/rustyhand --version && \
        echo "PASS: rustyhand binary works"; \
    else \
